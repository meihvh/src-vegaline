...
public final class ExclusionComposite extends RGBComposite {
   ...
   public CompositeContext createContext(ColorModel srcColorModel, ColorModel dstColorModel, RenderingHints hints) {
      return new ExclusionComposite.Context(this.extraAlpha, srcColorModel, dstColorModel);
   }
   ...
   static class Context extends RGBComposite.RGBCompositeContext {
      ...
      public void composeRGB(int[] src, int[] dst, float alpha) {
         ...
         for (int i = 0; i < w; i += 4) {
            ...
            int sa = src[i + 3];
            int dia = dst[i + 3];
            int dor = dir + multiply255(sr, 255 - dir - dir);
            int dog = dig + multiply255(sg, 255 - dig - dig);
            int dob = dib + multiply255(sb, 255 - dib - dib);
