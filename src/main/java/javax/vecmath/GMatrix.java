...
public class GMatrix implements Serializable, Cloneable {
   ...
   public final void copySubMatrix(int rowSource, int colSource, int numRow, int numCol, int rowDest, int colDest, GMatrix target) {
      if (this != target) {
         ...
         for (int var12 = 0; var12 < numRow; var12++) {
            for (int j = 0; j < numCol; j++) {
               target.values[rowDest + var12][colDest + j] = tmp[var12][j];
            }
         }
      }
   }
   ...
   public final void set(GMatrix m1) {
      ...
      for (int var4 = m1.nRow; var4 < this.nRow; var4++) {
         for (int j = m1.nCol; j < this.nCol; j++) {
            this.values[var4][j] = 0.0;
         }
      }
   }
   ...
   public final void get(GMatrix m1) {
      ...
      for (int var6 = nr; var6 < m1.nRow; var6++) {
         for (int j = 0; j < m1.nCol; j++) {
            m1.values[var6][j] = 0.0;
         }
      }

      for (int j = nc; j < m1.nCol; j++) {
         for (int var7 = 0; var7 < nr; var7++) {
            m1.values[var7][j] = 0.0;
         }
      }
   }
      ...
      if (this.nRow != this.nCol) {
         ...
         if (!luDecomposition(LU.nRow, temp, row_perm, even_row_exchange)) {
            ...
            for (int var9 = 0; var9 < this.nRow; var9++) {
               for (int j = 0; j < this.nCol; j++) {
                  LU.values[var9][j] = temp[var9 * this.nCol + j];
               }
            }

            for (int var10 = 0; var10 < LU.nRow; var10++) {
               permutation.values[var10] = (double)row_perm[var10];
            }
      ...
      if (m1.nRow != m1.nCol) {
         ...
         if (!luDecomposition(m1.nRow, temp, row_perm, even_row_exchange)) {
            ...
            for (int var10 = 0; var10 < this.nCol; var10++) {
               result[var10 + var10 * this.nCol] = 1.0;
            }
            ...
            for (int var11 = 0; var11 < this.nRow; var11++) {
               for (int j = 0; j < this.nCol; j++) {
                  this.values[var11][j] = result[var11 * this.nCol + j];
               }
            }
         }
      }
   ...
   static boolean luDecomposition(int dim, double[] matrix0, int[] row_perm, int[] even_row_xchg) {
      ...
      for (int j = 0; j < dim; j++) {
         for (int var21 = 0; var21 < j; var21++) {
            int target = mtx + dim * var21 + j;
            double sum = matrix0[target];
            int k = var21;
            int p1 = mtx + dim * var21;

            for (int p2 = mtx + j; k-- != 0; p2 += dim) {
         ...
         for (int var22 = j; var22 < dim; var22++) {
            int target = mtx + dim * var22 + j;
            double sum = matrix0[target];
            int k = j;
            int p1 = mtx + dim * var22;
            ...
            matrix0[target] = sum;
            double temp;
            if ((temp = row_scale[var22] * Math.abs(sum)) >= big) {
               big = temp;
               imax = var22;
            }
         }
         ...
         if (j != dim - 1) {
            double temp = 1.0 / matrix0[mtx + dim * j + j];
            int target = mtx + dim * (j + 1) + j;

            for (int var23 = dim - 1 - j; var23-- != 0; target += dim) {
               matrix0[target] *= temp;
            }
         }
      }
   ...
   static void luBacksubstitution(int dim, double[] matrix1, int[] row_perm, double[] matrix2) {
      ...
      for (int k = 0; k < dim; k++) {
         ...
         for (int var17 = 0; var17 < dim; var17++) {
            int ri = dim - 1 - var17;
            int rv = dim * ri;
            double tt = 0.0;

            for (int j = 1; j <= var17; j++) {
   ...
   static int computeSVD(GMatrix mat, GMatrix U, GMatrix W, GMatrix V) {
      ...
      for (int si = 0; si < sLength; si++) {
         if (nr > 1) {
            ...
            for (int var38 = 1; var38 < nr; var38++) {
               vec[var38] = m.values[si + var38][si];
            }

            double scale = 0.0;

            for (int var39 = 0; var39 < nr; var39++) {
               scale += vec[var39] * vec[var39];
            }
            ...
            for (int var40 = si; var40 < m.nRow; var40++) {
               u.values[var40][var40]++;
            }

            double t = 0.0;

            for (int var41 = si; var41 < m.nRow; var41++) {
               t += u.values[si][var41] * m.values[var41][si];
            }

            m.values[si][si] = t;

            for (int var53 = si; var53 < m.nRow; var53++) {
               for (int k = si + 1; k < m.nCol; k++) {
                  tmp.values[var53][k] = 0.0;

                  for (int var42 = si; var42 < m.nCol; var42++) {
                     tmp.values[var53][k] = tmp.values[var53][k] + u.values[var53][var42] * m.values[var42][k];
                  }
               }
            }

            for (int var54 = si; var54 < m.nRow; var54++) {
               for (int k = si + 1; k < m.nCol; k++) {
                  m.values[var54][k] = tmp.values[var54][k];
               }
            }

            for (int var55 = si; var55 < m.nRow; var55++) {
               for (int k = 0; k < m.nCol; k++) {
                  tmp.values[var55][k] = 0.0;

                  for (int var43 = si; var43 < m.nCol; var43++) {
                     tmp.values[var55][k] = tmp.values[var55][k] + u.values[var55][var43] * U.values[var43][k];
                  }
               }
            }

            for (int var56 = si; var56 < m.nRow; var56++) {
               for (int k = 0; k < m.nCol; k++) {
                  U.values[var56][k] = tmp.values[var56][k];
               }
            }
         ...
         if (nc > 2) {
            ...
            for (int var45 = 1; var45 < nc - 1; var45++) {
               vec[var45] = m.values[si][si + var45 + 1];
            }

            double scale = 0.0;

            for (int var46 = 0; var46 < nc - 1; var46++) {
               scale += vec[var46] * vec[var46];
            }
            ...
            for (int var47 = si + 1; var47 < m.nCol; var47++) {
               v.values[var47][var47]++;
            }

            double t = 0.0;

            for (int var48 = si; var48 < m.nCol; var48++) {
               t += v.values[var48][si + 1] * m.values[si][var48];
            }

            m.values[si][si + 1] = t;

            for (int var58 = si + 1; var58 < m.nRow; var58++) {
               for (int k = si + 1; k < m.nCol; k++) {
                  tmp.values[var58][k] = 0.0;

                  for (int var49 = si + 1; var49 < m.nCol; var49++) {
                     tmp.values[var58][k] = tmp.values[var58][k] + v.values[var49][k] * m.values[var58][var49];
                  }
               }
            }

            for (int var59 = si + 1; var59 < m.nRow; var59++) {
               for (int k = si + 1; k < m.nCol; k++) {
                  m.values[var59][k] = tmp.values[var59][k];
               }
            }

            for (int var60 = 0; var60 < m.nRow; var60++) {
               for (int k = si + 1; k < m.nCol; k++) {
                  tmp.values[var60][k] = 0.0;

                  for (int var50 = si + 1; var50 < m.nCol; var50++) {
                     tmp.values[var60][k] = tmp.values[var60][k] + v.values[var50][k] * V.values[var60][var50];
                  }
               }
            }

            for (int var61 = 0; var61 < m.nRow; var61++) {
               for (int k = si + 1; k < m.nCol; k++) {
                  V.values[var61][k] = tmp.values[var61][k];
               }
            }
      ...
      for (int var52 = 0; var52 < eLength; var52++) {
         e[var52] = m.values[var52][var52 + 1];
      }
   ...
   static void compute_qr(int start, int end, double[] s, double[] e, GMatrix u, GMatrix v) {
      ...
      for (int k = 0; k < 2 && !converged; k++) {
         ...
         if (s.length == e.length) {
            double rx = compute_rot(f, g, sinr, cosr);
            f = cosr[0] * s[i] + sinr[0] * e[i];
            e[i] = cosr[0] * e[i] - sinr[0] * s[i];
            s[i + 1] = cosr[0] * s[i + 1];
            update_v(i, v, cosr, sinr);
   ...
   private static void print_svd(double[] s, double[] e, GMatrix u, GMatrix v) {
      ...
      for (int var6 = 0; var6 < e.length; var6++) {
         System.out.println(" " + e[var6]);
      }
      ...
      for (int var7 = 0; var7 < s.length; var7++) {
         mtmp.values[var7][var7] = s[var7];
      }

      for (int var8 = 0; var8 < e.length; var8++) {
         mtmp.values[var8][var8 + 1] = e[var8];
      }
   ...
   static double compute_rot(double f, double g, double[] sin, double[] cos) {
      ...
      if (g == 0.0) {
         ...
         if (scale >= 4.994797680505588E145) {
            int count;
            for (count = 0; scale >= 4.994797680505588E145; scale = max(Math.abs(f1), Math.abs(g1))) {
               count++;
            ...
            int count;
            for (count = 0; scale <= 2.002083095183101E-146; scale = max(Math.abs(f1), Math.abs(g1))) {
               count++;
               f1 *= 4.994797680505588E145;
               g1 *= 4.994797680505588E145;
            }
