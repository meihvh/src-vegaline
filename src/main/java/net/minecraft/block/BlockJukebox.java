...
public class BlockJukebox extends BlockContainer {
   ...
   private void dropRecord(World worldIn, BlockPos pos, IBlockState state) {
      if (!worldIn.isRemote && worldIn.getTileEntity(pos) instanceof BlockJukebox.TileEntityJukebox blockjukebox$tileentityjukebox) {
         ItemStack itemstack = blockjukebox$tileentityjukebox.getRecord();
         if (!itemstack.func_190926_b()) {
            worldIn.playEvent(Minecraft.player, 1010, pos, 0);
            worldIn.playRecord(pos, null);
            blockjukebox$tileentityjukebox.setRecord(ItemStack.field_190927_a);
            float f = 0.7F;
            double d0 = (double)(worldIn.rand.nextFloat() * 0.7F) + 0.15F;
            double d1 = (double)(worldIn.rand.nextFloat() * 0.7F) + 0.060000002F + 0.6;
            double d2 = (double)(worldIn.rand.nextFloat() * 0.7F) + 0.15F;
