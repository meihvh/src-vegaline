      ...
      if (this.isDouble()) {
         ...
         if (flag1) {
            if (side == EnumFacing.DOWN) {
               ...
               return side == EnumFacing.UP && super.shouldSideBeRendered(blockState, blockAccess, pos, side) ? true : !isHalfSlab(iblockstate) || !flag;
            }
         } else if (side == EnumFacing.UP) {
            return true;
         } else {
            return side == EnumFacing.DOWN && super.shouldSideBeRendered(blockState, blockAccess, pos, side) ? true : !isHalfSlab(iblockstate) || flag;
         }
      }
   }
