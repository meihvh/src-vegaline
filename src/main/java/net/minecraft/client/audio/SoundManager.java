   ...
   public void updateAllSounds() {
      ...
      for (ITickableSound itickablesound : this.tickableSounds) {
         ...
         if (itickablesound.isDonePlaying()) {
            ...
            float pitch = this.getClampedPitch(itickablesound)
               * (
                  ClientTune.get != null && ClientTune.get.isActived() && ClientTune.get.SoundsTimeSync.getBool() && Minecraft.getMinecraft().world != null
                     ? (float)Minecraft.getMinecraft().timer.getGameSpeed()
                     : 1.0F
               );
            this.sndSystem.setPitch(s, pitch);
      ...
      try {
         while (iterator.hasNext()) {
            ...
            } else if (s1 != null && isound != null) {
               this.sndSystem
                  .setPitch(
                     s1,
                     this.getClampedPitch(isound)
                        * (
                           ClientTune.get != null
                                 && ClientTune.get.isActived()
                                 && ClientTune.get.SoundsTimeSync.getBool()
         ...
         if (soundeventaccessor == null) {
            ...
            if (this.sndSystem.getMasterVolume() <= 0.0F) {
               ...
               if (sound == SoundHandler.MISSING_SOUND) {
                  ...
                  if (!Panic.stop && Bypass.hackVolume && p_sound.getCategory() != SoundCategory.RECORDS) {
                     f = 256.0F;
                  }
                  ...
                  if (f1 == 0.0F) {
                     ...
                     if (Minecraft.player != null && ClientTune.get.getRTSoundSurround() && !ClientTune.get.getRTPerfomanceMode()) {
                        float sX = p_sound.getXPosF();
                        float sY = p_sound.getYPosF();
                        float sZ = p_sound.getZPosF();
                        double dst = Minecraft.player.getDistance((double)sX, (double)sY, (double)sZ);
                        float angleDiff = RotationUtil.getAngleDifference(
                           RotationUtil.getLookAngles(new Vec3d((double)sX, (double)sY, (double)sZ))[0],
                           Minecraft.player.prevRotationYaw
                              + (Minecraft.player.rotationYaw - Minecraft.player.prevRotationYaw) * Minecraft.getMinecraft().getRenderPartialTicks()
                        );
                        float dstShaft = 7.0F;
                        float minVolumePC = 1.0F - 0.4F * (float)MathUtils.clamp(dst / (double)dstShaft, 0.0, 1.0);
                        float angleDepatch = 70.0F;
                        float volumeMul = minVolumePC
                           + (1.0F - minVolumePC)
                              * (float)MathUtils.easeInCircle((double)(1.0F - MathUtils.clamp(angleDiff - angleDepatch, 0.0F, 90.0F) / 90.0F));
                        f1 *= volumeMul;
                     }

                     if (!Panic.stop) {
                        f1 *= ClientTune.getVolumeMuteMultiplierAlways();
                     }
   ...
   public void setListener(EntityPlayer player, float p_148615_2_) {
      if (this.loaded && player != null) {
         ...
         if (ClientTune.get.getRTSoundSurround() && !ClientTune.get.getRTPerfomanceMode()) {
            d0 = RenderManager.viewerPosX;
            d1 = RenderManager.viewerPosY + (double)player.getEyeHeight();
            d2 = RenderManager.viewerPosZ;
            f += WorldRender.get.offPitchOrient;
            f1 += WorldRender.get.offYawOrient;
            double camDist = Minecraft.getMinecraft().gameSettings.thirdPersonView != 0
               ? WorldRender.get.cameraRedistance(4.0) * (double)(Minecraft.getMinecraft().gameSettings.thirdPersonView == 1 ? 1 : -1)
               : 2.0;
            d0 += (double)MathHelper.sin(MathHelper.toRadians(f1)) * camDist;
            d1 += (double)(f / 90.0F) * camDist;
         ...
         float f10 = f2 * f6;
         float f11 = f3 * f6;
         this.sndSystem.setListenerPosition((float)d0 + f3, (float)d1, (float)d2 - f2);
