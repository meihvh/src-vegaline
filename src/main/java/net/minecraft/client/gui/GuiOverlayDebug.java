...
public class GuiOverlayDebug extends Gui {
   ...
   protected void renderDebugInfoLeft() {
      ...
      if (!Panic.stop && ComfortUi.get.isBetterDebugF3()) {
         ...
         GL11.glPushMatrix();
         GL11.glTranslated((double)((float)(-sr.getScaledWidth() / 4) * GuiIngame.sizer), (double)(-GuiIngame.sizer * 64.0F), 0.0);

         for (int i = 0; i < list.size(); i++) {
            String s = list.get(i);
            if (!Strings.isNullOrEmpty(s)) {
               int j = 8;
               int k = (int)Fonts.mntsb_15.getStringWidth(s);
               int l = 2;
               int i1 = 2 + j * i;
               int color = 14737632;
               if (s.startsWith("XYZ:")) {
                  ...
                  RenderUtils.drawGradientSideways(
                     ...
                     (double)(2 + k + 1),
                     (double)(i1 + j - 1),
                     ColorUtils.getColor(155, 255, 225, 70),
                     ColorUtils.getColor(0, 0, 125, 0)
                  );
               } else {
                  RenderUtils.drawGradientSideways(1.0, (double)(i1 - 1), (double)(2 + k + 1), (double)(i1 + j - 1), ColorUtils.getColor(210, 210, 225, 110), 0);
               }

               Fonts.mntsb_12.drawString(s, 2.0F, (float)(i1 + 2), color);
            }
         }
         ...
         for (int ix = 0; ix < list.size(); ix++) {
            String s = list.get(ix);
            if (!Strings.isNullOrEmpty(s)) {
               int j = this.fontRenderer.FONT_HEIGHT;
               int k = this.fontRenderer.getStringWidth(s);
               int l = 2;
               int i1 = 2 + j * ix;
               drawRect(1, (double)(i1 - 1), (double)(2 + k + 1), (double)(i1 + j - 1), -1873784752);
               this.fontRenderer.drawString(s, 2.0F, (double)i1, 14737632);
            }
         }
      }
   }
   ...
   protected void renderDebugInfoRight(ScaledResolution scaledRes) {
      ...
      if (!Panic.stop && ComfortUi.get.isBetterDebugF3()) {
         ...
         for (int i = 0; i < list.size(); i++) {
            ...
            if (!Strings.isNullOrEmpty(s)) {
               int j = 8;
               int k = (int)Fonts.mntsb_12.getStringWidth(s);
               int l = scaledRes.getScaledWidth() - 2 - k;
               int i1 = 2 + j * i;
               RenderUtils.drawGradientSideways(
                  (double)(l - 1), (double)(i1 - 1), (double)(l + k + 1), (double)(i1 + j - 1), 0, ColorUtils.getColor(210, 210, 225, 110)
               );
               Fonts.mntsb_12.drawString(s, (float)l, (float)(i1 + 2), 14737632);
            }
         }
         ...
         for (int ix = 0; ix < list.size(); ix++) {
            ...
            if (!Strings.isNullOrEmpty(s)) {
               ...
               int i1 = 2 + j * ix;
               drawRect(l - 1, (double)(i1 - 1), (double)(l + k + 1), (double)(i1 + j - 1), -1873784752);
               this.fontRenderer.drawString(s, (float)l, (double)i1, 14737632);
            }
         }
      }
   }
   ...
   protected List<String> call() {
      ...
      if (this.mc.isReducedDebug()) {
         return Lists.newArrayList(
            ClientBrandRetriever.getDebugVersionString(),
            this.mc.debug,
         ...
         if (this.mc.world != null) {
            ...
            if (!this.mc.world.isBlockLoaded(blockpos) || blockpos.getY() < 0 || blockpos.getY() >= 256) {
               ...
               if (this.mc.isIntegratedServerRunning() && this.mc.getIntegratedServer() != null) {
                  EntityPlayerMP entityplayermp = this.mc.getIntegratedServer().getPlayerList().getPlayerByUUID(Minecraft.player.getUniqueID());
