...
public class GuiScreenOptionsSounds extends GuiScreen {
   ...
   public void initGui() {
      ...
      this.buttonList
         .add(
            new GuiScreenOptionsSounds.Button(
               SoundCategory.MASTER.ordinal(), width / 2 - 155 + i % 2 * 160, height / 6 - 12 + 24 * (i >> 1), SoundCategory.MASTER, true
            )
         );
      i += 2;
      ...
      for (SoundCategory soundcategory : SoundCategory.values()) {
         if (soundcategory != SoundCategory.MASTER) {
            this.buttonList
               .add(
                  new GuiScreenOptionsSounds.Button(
                     soundcategory.ordinal(), width / 2 - 155 + i % 2 * 160, height / 6 - 12 + 24 * (i >> 1), soundcategory, false
                  )
               );
            i++;
         }
      ...
      this.buttonList
         .add(
            new GuiOptionButton(
               201, j, k + 24 * (++i >> 1), GameSettings.Options.SHOW_SUBTITLES, this.game_settings_4.getKeyBinding(GameSettings.Options.SHOW_SUBTITLES)
            )
         );
