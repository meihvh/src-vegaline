...
public class GuiBeacon extends GuiContainer {
      ...
      if (this.buttonsNotDrawn && i >= 0) {
         ...
         for (int k = 0; k <= 2; k++) {
            ...
            for (int j1 = 0; j1 < l; j1++) {
               ...
               GuiBeacon.PowerButton guibeacon$powerbutton = new GuiBeacon.PowerButton(
                  j++, this.guiLeft + 76 + j1 * 24 - i1 / 2, this.guiTop + 22 + k * 25, potion2, k
               );
         ...
         if (potion != null) {
            GuiBeacon.PowerButton guibeacon$powerbutton1 = new GuiBeacon.PowerButton(
               j++, this.guiLeft + 167 + (l1 - 1) * 24 - i2 / 2, this.guiTop + 47, potion, 3
            );
   ...
   public void actionPerformed(GuiButton button) throws IOException {
      if (button.id == -2) {
         ...
         Minecraft.player.connection.sendPacket(new CPacketCloseWindow(Minecraft.player.openContainer.windowId));
         this.mc.displayGuiScreen(null);
      } else if (button instanceof GuiBeacon.PowerButton guibeacon$powerbutton) {
