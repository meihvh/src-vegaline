...
public class ModelBoat extends ModelBase implements IMultipassModel {
   ...
   protected void renderPaddle(EntityBoat boat, int paddle, float scale, float limbSwing) {
      ...
      modelrenderer.rotateAngleX = (float)MathHelper.clampedLerp((float) (-Math.PI / 3), (float) (-Math.PI / 12), (double)((MathHelper.sin(-f) + 1.0F) / 2.0F));
      modelrenderer.rotateAngleY = (float)MathHelper.clampedLerp(-Math.PI / 4, Math.PI / 4, (double)((MathHelper.sin(-f + 1.0F) + 1.0F) / 2.0F));
      if (paddle == 1) {
