...
public class PlayerControllerMP {
   ...
   float blockReachInCGT = 5.0F;
   float blockReachInAGT = 4.5F;
   ...
   public static void clickBlockCreative(Minecraft mcIn, PlayerControllerMP playerController, BlockPos pos, EnumFacing facing) {
      if (!mcIn.world.extinguishFire(Minecraft.player, pos, facing) && !playerController.onPlayerDestroyBlock(pos) && PlayerHelper.get != null) {
         PlayerHelper.get.cantCancelDestroyAsSword(pos, facing);
      }
   }
      ...
      if (this.currentGameType.isCreative()
         ...
         if ((block instanceof BlockCommandBlock || block instanceof BlockStructure) && !Minecraft.player.canUseCommandBlock()) {
            ...
            if (flag) {
               ...
               if (!Panic.stop && Nuker.get.isActived()) {
                  Nuker.get.resetRenderHittingProgress();
               }
            }
   ...
   public boolean onPlayerDamageBlock(BlockPos posBlock, EnumFacing directionFacing) {
      ...
      if (this.blockHitDelay > 0) {
         ...
         if (iblockstate.getMaterial() == Material.AIR) {
            ...
            this.curBlockDamageMP = this.curBlockDamageMP + iblockstate.getPlayerRelativeBlockHardness(Minecraft.player, Minecraft.player.world, posBlock);
               ...
               this.mc
                  ...
                  .playSound(
                     new PositionedSoundRecord(
                        soundtype.getHitSound(), SoundCategory.NEUTRAL, (soundtype.getVolume() + 1.0F) / 8.0F, soundtype.getPitch() * 0.5F, posBlock
                     )
                  );
            }
   ...
   public float getBlockReachDistance() {
      return this.currentGameType.isCreative() ? this.blockReachInCGT : this.blockReachInAGT;
   }

   public void setBlockReachDistances(float inCreativeGameType, float inAnotherGameType) {
      this.blockReachInCGT = inCreativeGameType;
      this.blockReachInAGT = inAnotherGameType;
   }
   ...
   public EnumActionResult processRightClickBlock(EntityPlayerSP player, WorldClient worldIn, BlockPos stack, EnumFacing pos, Vec3d facing, EnumHand vec) {
      ...
      if (!this.mc.world.getWorldBorder().contains(stack)) {
         ...
         if (this.currentGameType != GameType.SPECTATOR) {
            ...
            if ((
                  !player.isSneaking()
                     || player.isNewSneak && !this.mc.gameSettings.keyBindSneak.isKeyDown()
                     || player.getHeldItemMainhand().func_190926_b() && player.getHeldItemOffhand().func_190926_b()
               )
               && iblockstate.getBlock().onBlockActivated(worldIn, stack, iblockstate, player, vec, pos, f, f1, f2)) {
               flag = true;
            }

            if (!flag && itemstack.getItem() instanceof ItemBlock itemblock && !itemblock.canPlaceBlockOnSide(worldIn, stack, pos, player, itemstack)) {
               return EnumActionResult.FAIL;
            }
         }
   ...
   public EnumActionResult processRightClick(EntityPlayer player, World worldIn, EnumHand stack) {
      if (this.currentGameType == GameType.SPECTATOR) {
         ...
         boolean stackCooledByClient = !Panic.stop
            && PlayerHelper.get.actived
            && (
               itemstack.getItem() instanceof ItemAppleGold && PlayerHelper.checkApple
                  || itemstack.getItem() instanceof ItemEnderPearl && PlayerHelper.checkPearl
                  || itemstack.getItem() instanceof ItemChorusFruit && PlayerHelper.checkChorus
            );
         if (!stackCooledByClient) {
            this.connection.sendPacket(new CPacketPlayerTryUseItem(stack));
         }

         if (itemstack.getItem() instanceof ItemEnderPearl pearl
            && Minecraft.player.getCooldownTracker().getCooldown(pearl, 0.0F) == 0.0F
            && !Panic.stop
            && PlayerHelper.get.actived
            && PlayerHelper.get.PearlThrowCooldown.getBool()
            && !PlayerHelper.checkPearl
            && PlayerHelper.canCooldown()) {
   ...
   public void attackEntity(EntityPlayer playerIn, Entity targetEntity) {
      if (Minecraft.temporalImageSizeMoreThan16x || !(Math.random() > 0.8F)) {
         if (!MiddleClick.get.NoHitFriend.getBool() || !Client.friendManager.isFriend(targetEntity.getName())) {
            if (Client.summit(targetEntity)) {
               Client.msg("А ты не обарзел?", true);
            } else {
               if (playerIn instanceof EntityPlayerSP) {
                  Bypass.get.onAttack(true);
   ...
   public ItemStack windowClick(int windowId, int slotId, int mouseButton, ClickType type, EntityPlayer player) {
      ...
      if ((!itemstack.func_190926_b() || type != ClickType.PICKUP) && Bypass.onWinClick()) {
         this.connection.sendPacket(new CPacketClickWindow(windowId, slotId, mouseButton, type, itemstack, short1), 100);
      } else {
   ...
   public void windowClickMemory(int windowId, int slotId, int mouseButton, ClickType type, EntityPlayer player, int timeWait) {
      if (player instanceof EntityPlayerSP playerSP) {
         playerSP.windowClickMemories.add(new EntityPlayerSP.WindowClickMemory(windowId, slotId, mouseButton, type, player, timeWait));
      } else {
         this.windowClick(windowId, slotId, mouseButton, type, player);
      }
   }
