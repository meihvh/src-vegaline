...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
   ...
   public void handleJoinGame(SPacketJoinGame packetIn) {
      ...
      if (Panic.stop || !WorldRender.get.isActived() || !WorldRender.get.FastWorldLoad.getBool() || Minecraft.player == null) {
      ...
      if (!Bypass.get.isActived() && !Panic.stop || !Bypass.get.ClientSpoof.getBool() || !Bypass.get.HideClient.getBool()) {
         this.netManager
   ...
   public void handleSpawnObject(SPacketSpawnObject packetIn) {
      ...
      if (packetIn.getType() == 10) {
         ...
         EntityEnderPearl pearl = new EntityEnderPearl(this.clientWorldController, d0, d1, d2);
         pearl.setFirstNeareblePlayer();
         entity = pearl;
   ...
   public void handleEntityVelocity(SPacketEntityVelocity packetIn) {
      ...
      if (this.gameController.world != null) {
         ...
         if (entity instanceof EntityPlayerSP && !Panic.stop && Velocity.get.actived && Velocity.get.OnKnockBack.getBool() && !Velocity.pass) {
   ...
   public void handleSpawnPlayer(SPacketSpawnPlayer packetIn) {
      ...
      if (this.getPlayerInfo(packetIn.getUniqueId()) != null && this.getPlayerInfo(packetIn.getUniqueId()).getGameProfile() != null) {
         EntityOtherPlayerMP entityotherplayermp = new EntityOtherPlayerMP(
            this.gameController.world, this.getPlayerInfo(packetIn.getUniqueId()).getGameProfile()
         );
   ...
   public void handlePlayerPosLook(SPacketPlayerPosLook packetIn) {
      ...
      if (entityplayer != null) {
         ...
         boolean hasPearlPerFlag = entityplayer instanceof EntityPlayerSP
            && Bypass.get.canFixPearlFlag()
            && MoveMeHelp.getCuttingSpeed() <= 0.25
            && Minecraft.getMinecraft()
               .world
               .getLoadedEntityList()
               .stream()
               .filter(entity -> entity instanceof EntityEnderPearl)
               .filter(Objects::nonNull)
               .anyMatch(pearl -> pearl.getDistanceToVec3d(new Vec3d(packetIn.getX(), packetIn.getY(), packetIn.getZ())) < 2.40112);
         ...
         if (!Panic.stop && hasPearlPerFlag) {
            ...
            Minecraft.player.motionX = MathUtils.getDifferenceOf(Minecraft.player.motionX, 0.0) < 0.12
               ? (Minecraft.player.motionX > 0.0 ? 0.12 : -0.12)
               : Minecraft.player.motionX;
            Minecraft.player.motionZ = -0.13 + 0.26 * Math.random();
            Minecraft.player.motionZ = MathUtils.getDifferenceOf(Minecraft.player.motionZ, 0.0) < 0.12
               ? (Minecraft.player.motionZ > 0.0 ? 0.12 : -0.12)
         ...
         if (!Panic.stop && Minecraft.player != null && Minecraft.getMinecraft().world != null && Minecraft.player == Minecraft.player) {
            Minecraft.getMinecraft()
               ...
               .forEach(
                  ents -> {
                     if (ents != null
                        && ents instanceof EntityEnderPearl pearl
                        && pearl.getDistanceToVec3d(new Vec3d(packetIn.getX(), packetIn.getY(), packetIn.getZ())) < 3.40112) {
   ...
   public void handleMultiBlockChange(SPacketMultiBlockChange packetIn) {
      ...
      for (SPacketMultiBlockChange.BlockUpdateData spacketmultiblockchange$blockupdatedata : packetIn.getChangedBlocks()) {
         this.clientWorldController
            .invalidateRegionAndSetBlock(spacketmultiblockchange$blockupdatedata.getPos(), spacketmultiblockchange$blockupdatedata.getBlockState());
      }
   }
   ...
   public void onDisconnect(ITextComponent reason) {
      ...
      if (this.guiScreenServer != null) {
         if (this.guiScreenServer instanceof GuiScreenRealmsProxy) {
            this.gameController
               .displayGuiScreen(new DisconnectedRealmsScreen(((GuiScreenRealmsProxy)this.guiScreenServer).getProxy(), "disconnect.lost", reason).getProxy());
         } else {
   ...
   public void sendPacket(Packet<?> packetIn, int timeWait) {
      EventSendPacket DICK = new EventSendPacket(packetIn);
      DICK.call();
      if (packetIn != null && !DICK.isCancelled() && Minecraft.player != null) {
         Minecraft.player.sendPacketsMemories.add(new EntityPlayerSP.SendPacketMemory(packetIn, timeWait));
      }
   }
   ...
   public void handleCollectItem(SPacketCollectItem packetIn) {
      ...
      if (entity instanceof EntityItem EItem) {
      ...
      if (entity instanceof EntityItem EItemx) {
         if (Hud.get.isActived()) {
            Hud.get.onCollect(EItemx, entitylivingbase);
         }

         if (PickBlowup.get.isActived()) {
            PickBlowup.get.onCollect(EItemx, entitylivingbase);
         }
      }
            ...
            if (entity1 != null) {
               ...
               if (entity1 == Minecraft.player && !flag) {
                  this.gameController
                     .ingameGUI
                     .setRecordPlaying(
                        I18n.format("mount.onboard", GameSettings.getKeyDisplayString(this.gameController.gameSettings.keyBindSneak.getKeyCode())), false
                     );
               }
            }
         }
   ...
   public void handleEntityStatus(SPacketEntityStatus packetIn) {
      ...
      if (entity != null) {
         if (packetIn.getOpCode() == 21) {
            ...
            if (packetIn instanceof SPacketEntityStatus && packetIn.getOpCode() == 35) {
               if (packetIn.getEntity(Minecraft.getMinecraft().world) == Minecraft.player && OffHand.get.actived) {
                  OffHand.scaleAnim.setAnim(0.6F);
                  OffHand.scaleAnim.to = 1.05F;
                  OffHand.popAnim.setAnim(1.0F);
                  if (Minecraft.player.getTotalArmorValue() >= 10) {
               ...
               if (Minecraft.getMinecraft().world != null && packetIn.getEntity(Minecraft.getMinecraft().world) instanceof EntityLivingBase base) {
                  if (!(base instanceof EntityPlayerSP) && !NameTags.canCancelHandleWorldPotionEffects(Minecraft.getMinecraft().world.isRemote)) {
                     ...
                     base.setAbsorptionAmount(8.0F);
                  }

                  base.totemTick = true;
                  base.totemsPopped++;
                  base.totemTickOutHurt = 10;
               }
            }
            ...
            this.clientWorldController
               .playSound(entity.posX, entity.posY, entity.posZ, SoundEvents.field_191263_gW, entity.getSoundCategory(), 1.0F, 1.0F, false);
            if (entity == Minecraft.player && (!NoRender.get.actived || !NoRender.get.TotemOverlay.getBool())) {
               ItemStack stack = null;
               if (Minecraft.player.getHeldItemOffhand().getItem() == Items.TOTEM) {
                  stack = Minecraft.player.getHeldItemOffhand();
               } else if (Minecraft.player.getHeldItemMainhand().getItem() == Items.TOTEM) {
                  stack = Minecraft.player.getHeldItemMainhand();
               } else {
                  stack = new ItemStack(Items.TOTEM, 1);
               }
   ...
   public void handleRespawn(SPacketRespawn packetIn) {
      ...
      if (packetIn.getDimensionID() != Minecraft.player.dimension) {
         ...
         this.clientWorldController = new WorldClient(
            this,
            new WorldSettings(0L, packetIn.getGameType(), false, this.gameController.world.getWorldInfo().isHardcoreModeEnabled(), packetIn.getWorldType()),
   ...
   public void handleExplosion(SPacketExplosion packetIn) {
      ...
      Explosion explosion = new Explosion(
         this.gameController.world, null, packetIn.getX(), packetIn.getY(), packetIn.getZ(), packetIn.getStrength(), packetIn.getAffectedBlockPositions()
      );
      explosion.doExplosionB(true);
      if (Minecraft.player instanceof EntityPlayerSP && !Panic.stop && Velocity.get != null && Velocity.get.actived && Velocity.get.OnKnockBack.getBool()) {
         Velocity.abuseVelocity(Minecraft.player, new Vec3d((double)packetIn.getMotionX(), (double)packetIn.getMotionY(), (double)packetIn.getMotionZ()));
      } else {
         Minecraft.player.motionX = Minecraft.player.motionX + (double)packetIn.getMotionX();
         Minecraft.player.motionY = Minecraft.player.motionY + (double)packetIn.getMotionY();
         Minecraft.player.motionZ = Minecraft.player.motionZ + (double)packetIn.getMotionZ();
      }
   }
   ...
   public void handleOpenWindow(SPacketOpenWindow packetIn) {
      ...
      if ("minecraft:container".equals(packetIn.getGuiId())) {
         InventoryBasic inventoryBasic = new InventoryBasic(packetIn.getWindowTitle(), packetIn.getSlotCount());
         boolean cancel = ChestChecker.get.onReceiveWindowCancelOpen(true, inventoryBasic, packetIn.getWindowId());
         entityplayersp.displayGUIChest(inventoryBasic);
         entityplayersp.openContainer.windowId = packetIn.getWindowId();
         if (cancel) {
            Minecraft.getMinecraft().currentScreen = null;
         }

         ChestChecker.get.onReceiveWindowCancelOpen(false, inventoryBasic, packetIn.getWindowId());
         ...
         boolean cancel = PotionBrewer.get.onReceiveWindowCancelOpen(true, iinventory, packetIn.getWindowId());
         boolean cancel2 = ChestChecker.get.onReceiveWindowCancelOpen(true, iinventory, packetIn.getWindowId());
         ...
         PotionBrewer.get.onReceiveWindowCancelOpen(false, iinventory, packetIn.getWindowId());
         ChestChecker.get.onReceiveWindowCancelOpen(false, iinventory, packetIn.getWindowId());
      }
   }
   ...
   public void handleSetSlot(SPacketSetSlot packetIn) {
      ...
      if (packetIn.getStack().getItem() instanceof ItemArmor armor && packetIn.getStack().getDisplayName().contains("1.16 Netherite")) {
         armor.updateMaterial(ItemArmor.ArmorMaterial.NETHERITE);
      }
   ...
   public void handleWindowItems(SPacketWindowItems packetIn) {
      ...
      if (packetIn.getWindowId() == 0) {
         ...
         ChestStealer.get.onStacksUpdate();
      }
   }
   ...
   public void handleChangeGameState(SPacketChangeGameState packetIn) {
      ...
      if (i == 1) {
         ...
         if (j == 0) {
            ...
            this.gameController
               .displayGuiScreen(
                  new GuiWinGame(true, () -> Minecraft.player.connection.sendPacket(new CPacketClientStatus(CPacketClientStatus.State.PERFORM_RESPAWN)))
               );
         }
      } else if (i == 5) {
         ...
         if (f == 0.0F) {
            ...
            this.gameController
               ...
               .printChatMessage(
                  new TextComponentTranslation(
                     ...
                     GameSettings.getKeyDisplayString(Minecraft.getMinecraft().gameSettings.keyBindLeft.getKeyCode()),
                     GameSettings.getKeyDisplayString(gamesettings.keyBindBack.getKeyCode()),
                     GameSettings.getKeyDisplayString(Minecraft.getMinecraft().gameSettings.keyBindRight.getKeyCode())
                  )
               );
         } else if (f == 102.0F) {
            this.gameController
            ...
            this.gameController
               ...
               .getChatGUI()
               .printChatMessage(
                  new TextComponentTranslation("demo.help.inventory", GameSettings.getKeyDisplayString(gamesettings.keyBindInventory.getKeyCode()))
               );
         }
      } else if (i == 6) {
         this.clientWorldController
   ...
   public void handleSoundEffect(SPacketSoundEffect packetIn) {
      if (packetIn != null
         ...
         && packetIn.getCategory() == SoundCategory.BLOCKS
         && packetIn.getSound().getSoundName().getResourcePath().contains("block.brewing_stand.brew")) {
         PotionBrewer.get.onBrewSound(packetIn.getX(), packetIn.getY(), packetIn.getZ());
      }

      PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
      if ((ClientTune.get == null || Panic.stop || !ClientTune.get.onSoundIsCancel(packetIn)) && this.gameController.world != null) {
         this.gameController
            .world
            .playSound(
               Minecraft.player,
   ...
   public void handleCustomSound(SPacketCustomSound packetIn) {
      if (packetIn != null
         && packetIn.getSoundName() != null
         && packetIn.getCategory() == SoundCategory.BLOCKS
         && packetIn.getSoundName().contains("block.brewing_stand.brew")) {
         PotionBrewer.get.onBrewSound(packetIn.getX(), packetIn.getY(), packetIn.getZ());
      }

      PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
      if (ClientTune.get == null || Panic.stop || !ClientTune.get.onSoundIsCancel(packetIn)) {
         this.gameController
            .getSoundHandler()
            .playSound(
   ...
   public FutureCallback<Object> createDownloadCallback() {
   ...
   public void func_194307_a(SPacketPlaceGhostRecipe p_194307_1_) {
      ...
      if (container.windowId == p_194307_1_.func_194313_b()
         && container.getCanCraft(Minecraft.player)
         && this.gameController.currentScreen instanceof IRecipeShownListener) {
