...
public class ItemRenderer {
   ...
   public static boolean isRender3dHand;
   ...
   public void renderItemSide(EntityLivingBase entitylivingbaseIn, ItemStack heldStack, ItemCameraTransforms.TransformType transform, boolean leftHanded) {
      if (!heldStack.func_190926_b()) {
         ...
         if (entitylivingbaseIn.isChild()
            && entitylivingbaseIn.isSneaking()
            && (this.mc.gameSettings.thirdPersonView != 0 || !(entitylivingbaseIn instanceof EntityPlayerSP))) {
   ...
   private void setLightmap() {
      ...
      int i = this.mc
         .world
         .getCombinedLight(
            new BlockPos(abstractclientplayer.posX, abstractclientplayer.posY + (double)abstractclientplayer.getEyeHeight(), abstractclientplayer.posZ), 0
         );
   ...
   private void renderArm(EnumHandSide p_187455_1_) {
      ...
      Render<AbstractClientPlayer> render = this.renderManager.getEntityRenderObject(Minecraft.player);
   ...
   public void transformFirstPerson(EnumHandSide p_187453_1_, float p_187453_2_) {
      EventTransformSideFirstPerson event = new EventTransformSideFirstPerson(p_187453_1_, false);
      event.call();
      float f = MathHelper.sin(p_187453_2_ * p_187453_2_ * (float) Math.PI);
      float f1 = MathHelper.sin(MathHelper.sqrt(p_187453_2_) * (float) Math.PI);
      int i = p_187453_1_ == EnumHandSide.RIGHT ? 1 : -1;
      EnumHand eHand = Minecraft.player.getPrimaryHand() == EnumHandSide.RIGHT ? EnumHand.MAIN_HAND : EnumHand.OFF_HAND;
      if (!this.mc.runScreenshot && !ViewModel.animate(false, f, f1, i)) {
   ...
   private void transformSideFirstPerson(EnumHandSide p_187459_1_, float p_187459_2_) {
      ...
      float f = MathHelper.sin(p_187459_2_ * p_187459_2_ * (float) Math.PI);
      float f1 = MathHelper.sin(MathHelper.sqrt(p_187459_2_) * (float) Math.PI);
      if (!ViewModel.animateIfShield(false, f, f1, i)) {
         GlStateManager.translate(
            (float)i * 0.56F, -0.52F + p_187459_2_ * -0.6F * (float)(!this.mc.runScreenshot && ViewModel.animate(true, 0.0F, 0.0F, 0) ? 0 : 1), -0.72F
         );
      }
   }
   ...
   public void renderItemInFirstPerson(float partialTicks) {
      ...
      this.setLightmap();
      if (!NoRender.get.actived || !NoRender.get.HandShake.getBool()) {
      ...
      if (flag) {
         ...
         if (!Reflector.ForgeHooksClient_renderSpecificFirstPersonHand.exists()
            || !Reflector.callBoolean(
               Reflector.ForgeHooksClient_renderSpecificFirstPersonHand, EnumHand.MAIN_HAND, partialTicks, f1, f3, f5, this.itemStackMainHand
            )) {
      ...
      if (flag1) {
         ...
         if (!Reflector.ForgeHooksClient_renderSpecificFirstPersonHand.exists()
            || !Reflector.callBoolean(
               Reflector.ForgeHooksClient_renderSpecificFirstPersonHand, EnumHand.OFF_HAND, partialTicks, f1, f4, f6, this.itemStackOffHand
            )) {
      ...
      if (!Config.isShaders() || !Shaders.isSkipRenderHand(p_187457_4_)) {
         ...
         if (p_187457_6_.func_190926_b()) {
            ...
            if (p_187457_1_.isHandActive() && p_187457_1_.getItemInUseCount() > 0 && p_187457_1_.getActiveHand() == p_187457_4_) {
               int j = flag1 ? 1 : -1;
               EventTransformSideFirstPerson event = new EventTransformSideFirstPerson(enumhandside, true);
               ...
               int i = flag1 ? 1 : -1;
               if (!this.mc.runScreenshot && !ViewModel.animate(true, 0.0F, 0.0F, 0)) {
   ...
   public void renderOverlays(float partialTicks) {
      ...
      if (!Minecraft.player.isSpectator()) {
         if (Minecraft.player.isInsideOfMaterial(Material.WATER)
            && !Reflector.callBoolean(Reflector.ForgeEventFactory_renderWaterOverlay, Minecraft.player, partialTicks)
            && (!NoRender.get.actived || !NoRender.get.LiquidOverlay.getBool())) {
            this.renderWaterOverlayTexture(partialTicks);
         }

         if (Minecraft.player.isBurning()
            && !Reflector.callBoolean(Reflector.ForgeEventFactory_renderFireOverlay, Minecraft.player, partialTicks)
            && (!NoRender.get.actived || !NoRender.get.Fire.getBool())) {
      ...
      if (entityplayersp.isRowingBoat()) {
         ...
         if (Reflector.ForgeHooksClient_shouldCauseReequipAnimation.exists()) {
            boolean flag = Reflector.callBoolean(
               Reflector.ForgeHooksClient_shouldCauseReequipAnimation, this.itemStackMainHand, itemstack, entityplayersp.inventory.currentItem
            );
            ...
            equippedProgressMainHand = equippedProgressMainHand + MathHelper.clamp((!flag ? f * f * f : 0.0F) - equippedProgressMainHand, -0.4F, 0.4F);
            this.equippedProgressOffHand = this.equippedProgressOffHand + MathHelper.clamp((float)(!flag1 ? 1 : 0) - this.equippedProgressOffHand, -0.4F, 0.4F);
         } else {
            equippedProgressMainHand = equippedProgressMainHand
               + MathHelper.clamp((Objects.equals(this.itemStackMainHand, itemstack) ? f * f * f : 0.0F) - equippedProgressMainHand, -0.4F, 0.4F);
