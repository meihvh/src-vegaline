...
public class ModelBakery {
   ...
   public IRegistry<ModelResourceLocation, IBakedModel> setupModelRegistry() {
      this.loadBlocks();
      Minecraft.stage("load blocks");
      this.loadVariantItemModels();
      Minecraft.stage("load item models");
      this.loadModelsCheck();
      Minecraft.stage("check load models");
      this.loadSprites();
      Minecraft.stage("load sprites");
      this.makeItemModels();
      Minecraft.stage("make item models");
      this.bakeBlockModels();
      Minecraft.stage("load bake block models");
      this.bakeItemModels();
      Minecraft.stage("load bake item models");
      ...
      for (Block block : Block.REGISTRY) {
         for (ResourceLocation resourcelocation : blockstatemapper.getBlockstateLocations(block)) {
            try {
               ...
               LOGGER.warn("Unable to load definition " + resourcelocation, (Throwable)var7);
            }
         }
      }
   }
      ...
      for (Entry<IBlockState, ModelResourceLocation> entry : map.entrySet()) {
         ...
         if (p_loadBlock_3_.equals(modelresourcelocation)) {
            try {
               ...
               if (!modelblockdefinition.hasMultipartData()) {
                  LOGGER.warn("Unable to load variant: " + modelresourcelocation.getVariant() + " from " + modelresourcelocation, (Throwable)var10);
               }
            }
         }
      }
   ...
   private void loadVariantItemModels() {
      this.variants
         .put(
            MODEL_MISSING,
            new VariantList(Lists.newArrayList(new Variant(new ResourceLocation(MODEL_MISSING.getResourcePath()), ModelRotation.X0_Y0, false, 1)))
         );
      Minecraft.stage("load texture variants");
      this.func_191401_d();
      Minecraft.stage("init item frame map textures");
      this.loadVariantModels();
      Minecraft.stage("load variant models");
      this.loadMultipartVariantModels();
      Minecraft.stage("load multipart map");
      this.loadItemModels();
      Minecraft.stage("load item models");
      CustomItems.update();
      Minecraft.stage("update custom items");
      ...
      try {
         ...
         if (Reflector.ForgeModelBlockDefinition_parseFromReader2.exists()) {
            modelblockdefinition = (ModelBlockDefinition)Reflector.call(
               Reflector.ForgeModelBlockDefinition_parseFromReader2, new InputStreamReader(inputstream, StandardCharsets.UTF_8), location
            );
         } else {
   ...
   private void registerVariantNames() {
      ...
      this.variantNames
         .put(
            Item.getItemFromBlock(Blocks.field_192443_dR),
            Lists.newArrayList(
               "black_concrete",
               "red_concrete",
               "green_concrete",
      ...
      this.variantNames
         .put(
            Item.getItemFromBlock(Blocks.field_192444_dS),
            Lists.newArrayList(
