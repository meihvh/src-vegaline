...
public class DebugRendererPathfinding implements DebugRenderer.IDebugRenderer {
         ...
         for (Integer integer : this.pathMap.keySet()) {
            ...
            if (this.addDistanceToPlayer(pathpoint) <= 40.0F) {
               RenderGlobal.renderFilledBox(
                  new AxisAlignedBB(
                        (double)((float)pathpoint.xCoord + 0.25F),
                        (double)((float)pathpoint.yCoord + 0.25F),
                        (double)pathpoint.zCoord + 0.25,
                        (double)((float)pathpoint.xCoord + 0.75F),
                        (double)((float)pathpoint.yCoord + 0.75F),
                        (double)((float)pathpoint.zCoord + 0.75F)
                     )
            ...
            for (PathPoint pathpoint3 : path1.getClosedSet()) {
               if (this.addDistanceToPlayer(pathpoint3) <= 40.0F) {
                  DebugRenderer.renderDebugText(
                     ...
                     (double)pathpoint3.xCoord + 0.5,
                     (double)pathpoint3.yCoord + 0.75,
                     (double)pathpoint3.zCoord + 0.5,
                     p_190060_1_,
                     -65536
                  );
                  DebugRenderer.renderDebugText(
            ...
            for (PathPoint pathpoint4 : path1.getOpenSet()) {
               if (this.addDistanceToPlayer(pathpoint4) <= 40.0F) {
                  DebugRenderer.renderDebugText(
                     ...
                     (double)pathpoint4.xCoord + 0.5,
                     (double)pathpoint4.yCoord + 0.75,
                     (double)pathpoint4.zCoord + 0.5,
                     p_190060_1_,
                     -16776961
                  );
                  DebugRenderer.renderDebugText(
            ...
            for (int k = 0; k < path1.getCurrentPathLength(); k++) {
               ...
               if (this.addDistanceToPlayer(pathpoint2) <= 40.0F) {
                  DebugRenderer.renderDebugText(
                     ...
                     (double)pathpoint2.xCoord + 0.5,
                     (double)pathpoint2.yCoord + 0.75,
                     (double)pathpoint2.zCoord + 0.5,
                     p_190060_1_,
                     -1
                  );
                  DebugRenderer.renderDebugText(
   ...
   public void renderPathLine(float p_190067_1_, Path p_190067_2_) {
      ...
      for (int i = 0; i < p_190067_2_.getCurrentPathLength(); i++) {
         ...
         if (this.addDistanceToPlayer(pathpoint) <= 40.0F) {
            ...
            int l = j >> 8 & 0xFF;
            int i1 = j & 0xFF;
            bufferbuilder.pos((double)pathpoint.xCoord - this.xo + 0.5, (double)pathpoint.yCoord - this.yo + 0.5, (double)pathpoint.zCoord - this.zo + 0.5)
               .color(k, l, i1, 255)
               .endVertex();
         }
      }
   ...
   private float addDistanceToPlayer(PathPoint p_190066_1_) {
      return (float)(
         Math.abs((double)p_190066_1_.xCoord - this.player.posX)
