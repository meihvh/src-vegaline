...
public class RenderCreeper extends RenderLiving<EntityCreeper> {
   ...
   protected void preRenderCallback(EntityCreeper entitylivingbaseIn, float partialTickTime) {
      ...
      if (BadTrip.get.isSpreadCreeper()) {
         f2 += 3.0F * f;
         GL11.glRotated((double)(f1 * f * 1200.0F + entitylivingbaseIn.getCreeperFlashIntensity(partialTickTime) * 720.0F), 0.0, 1.0, 0.0);
      }
   ...
   protected int getColorMultiplier(EntityCreeper entitylivingbaseIn, float lightBrightness, float partialTickTime) {
      ...
      if (BadTrip.get.isSpreadCreeper()) {
         f *= 8.0F;
         f %= 2.0F;
         f = (float)MathUtils.easeInOutQuadWave((double)(f / 2.0F));
         int i = (int)(f * 255.0F);
         i = MathHelper.clamp(i, 0, 255);
         i = 255 - i;
         return i << 24 | 822083583;
      } else if ((int)(f * 10.0F) % 2 == 0) {
         return 0;
      } else {
