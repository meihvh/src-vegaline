...
public class RenderManager {
   ...
   public <T extends Entity> Render<T> getEntityRenderObject(Entity entityIn) {
      if (entityIn instanceof AbstractClientPlayer) {
         ...
         return renderplayer != null ? (Render)renderplayer : (Render)this.playerRenderer;
      } else {
   ...
   public void renderEntityStaticNoShadow(Entity entity, float partialTicks, boolean p_147936_3_) {
      ...
      double d0 = MathUtils.interpolate(entity.lastTickPosX, entity.posX, (double)partialTicks);
      double d1 = MathUtils.interpolate(entity.lastTickPosY, entity.posY, (double)partialTicks);
      double d2 = MathUtils.interpolate(entity.lastTickPosZ, entity.posZ, (double)partialTicks);
      float f = MathUtils.interpolateFloat(entity.prevRotationYaw, entity.rotationYaw, (double)partialTicks);
         ...
         if (render != null && this.renderEngine != null) {
            try {
               if (render instanceof RenderEntity) {
                  render.setRenderOutlines(this.renderOutlines && !RenderLivingBase.silentMode);
               }
   ...
   private void renderDebugBoundingBox(Entity entityIn, double x, double y, double z, float entityYaw, float partialTicks) {
      ...
      AxisAlignedBB axisalignedbb = entityIn.getEntityBoundingBoxCL();
      ...
      bufferbuilder.pos(x + vec3d.xCoord * 2.0, y + (double)entityIn.getEyeHeight() + vec3d.yCoord * 2.0, z + vec3d.zCoord * 2.0)
         .color(0, 0, 255, 255)
         .endVertex();
      tessellator.draw();
