...
public abstract class LayerArmorBase<T extends ModelBase> implements LayerRenderer<EntityLivingBase> {
   ...
   public void doRenderLayer(
      ...
      if (!this.cancelArmorLayers()) {
         this.renderArmorLayer(
            entitylivingbaseIn, limbSwing, limbSwingAmount, partialTicks, ageInTicks, netHeadYaw, headPitch, scale, EntityEquipmentSlot.CHEST
         );
   ...
   final boolean cancelGlint() {
      return NoRender.get.isActived() && NoRender.get.EnchGlintEffect.getBool() || RenderLivingBase.unsetRenderArmorLayerAdditions;
   }

   final boolean cancelArmorLayers() {
      return NoRender.get.isActived() && NoRender.get.ArmorLayers.getBool();
   }
      ...
      if (itemstack.getItem() instanceof ItemArmor && (itemarmor = (ItemArmor)itemstack.getItem()).getEquipmentSlot() == slotIn) {
         ...
         if (Reflector.ForgeHooksClient_getArmorTexture.exists()) {
            if (ReflectorForge.armorHasOverlay(itemarmor, itemstack)) {
               ...
               if (!RenderLivingBase.unsetRenderArmorLayerAdditions) {
                  GlStateManager.color(1.0F * f3, 1.0F * f4, 1.0F * f5, this.alpha);
               }
         ...
         switch (itemarmor.getArmorMaterial()) {
            case LEATHER:
               ...
               if (!RenderLivingBase.unsetRenderArmorLayerAdditions) {
                  GlStateManager.color(1.0F * f, 1.0F * f1, 1.0F * f2, this.alpha);
               }
   ...
   protected T getArmorModelHook(
      EntityLivingBase p_getArmorModelHook_1_, ItemStack p_getArmorModelHook_2_, EntityEquipmentSlot p_getArmorModelHook_3_, T p_getArmorModelHook_4_
   ) {
      return p_getArmorModelHook_4_;
   }

   public ResourceLocation getArmorResource(
      Entity p_getArmorResource_1_, ItemStack p_getArmorResource_2_, EntityEquipmentSlot p_getArmorResource_3_, String p_getArmorResource_4_
   ) {
      ...
      String var11;
      ResourceLocation resourcelocation = ARMOR_TEXTURE_RES_MAP.get(
         var11 = Reflector.callString(
      ...
      if (resourcelocation == null) {
         resourcelocation = new ResourceLocation(var11);
         ARMOR_TEXTURE_RES_MAP.put(var11, resourcelocation);
      }
