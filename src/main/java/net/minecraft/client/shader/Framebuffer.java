...
public class Framebuffer {
   ...
   public void framebufferRenderExt(int width, int height, boolean p_178038_3_) {
      if (OpenGlHelper.isFramebufferEnabled()) {
         ...
         tessellator.draw();
         boolean setBloomFramebufferDrawing = false;
         if (setBloomFramebufferDrawing) {
            int itersRadian = 12;
            int itersRadius = 16;
            int startIterAlpha = 20;
            int bright = 255;
            float radiusPixInterval = 2.0F;
            ...
            for (int yaw = 0; yaw < 360; yaw += (int)(360.0F / (float)itersRadian)) {
               float rad = MathHelper.toRadians((float)yaw);
               float sinI = -MathHelper.sin(rad);
               float cosI = MathHelper.cos(rad);

               for (float radius = startRadius; radius < startRadius + radiusPixInterval * (float)itersRadius; radius += radiusPixInterval) {
                  float aPCMul = 1.0F - Math.min(radius / (startRadius + radiusPixInterval * (float)itersRadius), 1.0F);
                  int alpha = (int)Math.min((double)startIterAlpha * MathUtils.easeOutCubic((double)aPCMul), 255.0);
                  GL11.glPushMatrix();
                  GL11.glTranslated((double)(sinI * radius), (double)(cosI * radius), 0.0);
                  bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
                  bufferbuilder.pos(0.0, (double)f1, 0.0).tex(0.0, 0.0).color(bright, bright, bright, alpha).endVertex();
                  bufferbuilder.pos((double)f, (double)f1, 0.0).tex((double)f2, 0.0).color(bright, bright, bright, alpha).endVertex();
                  bufferbuilder.pos((double)f, 0.0, 0.0).tex((double)f2, (double)f3).color(bright, bright, bright, alpha).endVertex();
                  bufferbuilder.pos(0.0, 0.0, 0.0).tex(0.0, (double)f3).color(bright, bright, bright, alpha).endVertex();
                  tessellator.draw();
                  GL11.glPopMatrix();
               }
            }
   ...
   public void framebufferRenderExtSideways(int width, int height, boolean p_178038_3_, float alphaPCLeft, float alphaPCRight) {
      if (OpenGlHelper.isFramebufferEnabled()) {
         GlStateManager.colorMask(true, true, true, false);
         GlStateManager.disableDepth();
         GlStateManager.depthMask(false);
         ...
         BufferBuilder bufferbuilder = tessellator.getBuffer();
         bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
         bufferbuilder.pos(0.0, (double)f1, 0.0).tex(0.0, 0.0).color(255.0F, 255.0F, 255.0F, 255.0F * alphaPCLeft).endVertex();
         bufferbuilder.pos((double)f, (double)f1, 0.0).tex((double)f2, 0.0).color(255.0F, 255.0F, 255.0F, 255.0F * alphaPCRight).endVertex();
         bufferbuilder.pos((double)f, 0.0, 0.0).tex((double)f2, (double)f3).color(255.0F, 255.0F, 255.0F, 255.0F * alphaPCRight).endVertex();
         bufferbuilder.pos(0.0, 0.0, 0.0).tex(0.0, (double)f3).color(255.0F, 255.0F, 255.0F, 255.0F * alphaPCLeft).endVertex();
         tessellator.draw();
         this.unbindFramebufferTexture();
         GlStateManager.depthMask(true);
         GlStateManager.colorMask(true, true, true, true);
   ...
   public void framebufferClearNoViewPortSet() {
