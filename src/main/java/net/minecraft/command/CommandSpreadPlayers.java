...
public class CommandSpreadPlayers extends CommandBase {
   ...
   private void spread(
      ...
      CommandSpreadPlayers.Position[] acommandspreadplayers$position = this.createInitialPositions(
         random, respectTeams ? this.getNumberOfTeams(p_110669_2_) : p_110669_2_.size(), d0, d1, d2, d3
      );
      ...
      if (acommandspreadplayers$position.length > 1) {
         sender.addChatMessage(
            new TextComponentTranslation("commands.spreadplayers.info." + (respectTeams ? "teams" : "players"), String.format("%.2f", d4), i)
         );
      }
   }
         ...
         for (int j = 0; j < p_110668_14_.length; j++) {
            ...
            for (int l = 0; l < p_110668_14_.length; l++) {
               if (j != l) {
                  ...
                  if (d1 < p_110668_2_) {
                     k++;
                     commandspreadplayers$position1.x = commandspreadplayers$position1.x + (commandspreadplayers$position2.x - commandspreadplayers$position.x);
                     commandspreadplayers$position1.z = commandspreadplayers$position1.z + (commandspreadplayers$position2.z - commandspreadplayers$position.z);
                  }
               }
            }
   ...
   private double setPlayerPositions(List<Entity> p_110671_1_, World worldIn, CommandSpreadPlayers.Position[] p_110671_3_, boolean p_110671_4_) {
      ...
      for (int j = 0; j < p_110671_1_.size(); j++) {
         ...
         entity.setPositionAndUpdate(
            ...
            (double)MathHelper.floor(commandspreadplayers$position.z) + 0.5
         );
   ...
   private CommandSpreadPlayers.Position[] createInitialPositions(
