   ...
   protected void updateEquipmentIfNeeded(EntityItem itemEntity) {
      ...
      if (flag && this.canEquipItem(itemstack)) {
         if (!itemstack1.func_190926_b() && (double)(this.rand.nextFloat() - 0.1F) < switch (entityequipmentslot.getSlotType()) {
            case HAND -> (double)this.inventoryHandsDropChances[entityequipmentslot.getIndex()];
            case ARMOR -> (double)this.inventoryArmorDropChances[entityequipmentslot.getIndex()];
            default -> 0.0;
         }) {
      ...
      for (EntityEquipmentSlot entityequipmentslot : EntityEquipmentSlot.values()) {
         if (entityequipmentslot.getSlotType() == EntityEquipmentSlot.Type.ARMOR) {
            ...
            if (!itemstack.func_190926_b() && this.rand.nextFloat() < 0.5F * f) {
               this.setItemStackToSlot(
                  entityequipmentslot, EnchantmentHelper.addRandomEnchantment(this.rand, itemstack, (int)(5.0F + f * (float)this.rand.nextInt(18)), false)
               );
            }
         }
      }
