...
public abstract class EntityLivingBase extends Entity {
   ...
   public AnimationUtils smoothPreHurtHealth = new AnimationUtils(20.0F, 20.0F, 0.1F);
   private float preHurtHealth;
   public boolean totemTick = false;
   public int totemsPopped = 0;
   public int totemTickOutHurt = 0;
   ...
   public EntityLivingBase(World worldIn) {
      ...
      this.preHurtHealth = this.getMaxHealth();
      this.smoothPreHurtHealth = new AnimationUtils(this.preHurtHealth, this.preHurtHealth, 0.1F);
   ...
   public boolean canEntityBeSeenCoords(double x, double y, double z) {
      return this.world.rayTraceBlocks(new Vec3d(this.posX, this.posY + (double)this.getEyeHeight(), this.posZ), new Vec3d(x, y, z), false, true, false)
         == null;
   }
      ...
      if (!this.world.isRemote && this.fallDistance > 3.0F && onGroundIn) {
         ...
         if (state.getMaterial() != Material.AIR) {
            ...
            ((WorldServer)this.world)
               .spawnParticle(EnumParticleTypes.BLOCK_DUST, this.posX, this.posY, this.posZ, i, 0.0, 0.0, 0.0, 0.15F, Block.getStateId(state));
         }
      }
   ...
   public void onEntityUpdate() {
      ...
      if (this.isEntityAlive()) {
         ...
         } else if (flag && !this.world.getWorldBorder().contains(this.getEntityBoundingBoxCLCompact())) {
   ...
   public boolean isChild() {
      if (this.child) {
         ...
         if (SchoolBoy.get.actived) {
            String targets = SchoolBoy.get.Targets.getMode();
            switch (targets) {
               case "FullAll":
                  return this != null;
               case "All":
               ...
               case "Friends":
                  if (this instanceof EntityOtherPlayerMP mp && Client.friendManager.isFriend(mp.getName())) {
                     return true;
                  }

                  return false;
               case "Self&Friends":
                  if (!(this instanceof EntityPlayerSP) && (FreeCam.fakePlayer == null || this != FreeCam.fakePlayer)) {
                     if (this instanceof EntityOtherPlayerMP mp && Client.friendManager.isFriend(mp.getName())) {
                        return true;
                     }
   ...
   public float getDeathAlpha() {
      return this != null && this.getHealth() == 0.0F ? 1.0F - (float)this.deathTime / 19.0F : 1.0F;
   }
   ...
   public final float getHealth() {
      ...
      try {
         hp = this.dataManager.get(HEALTH);
      } catch (Exception var3) {
         var3.printStackTrace();
      }

      if (this != null && hp > 0.0F) {
         float hp2 = this.getScoreboardHealth(this);
         if (hp2 != 0.0F) {
            hp = hp2;
         }
      }

      return hp == 1000.0F ? 20.0F : hp;
   }
   ...
   public final float getPreHurtSmoothHealth() {
      return this.smoothPreHurtHealth.getAnim();
   }

   public final float getPreHurtHealth() {
      return this.preHurtHealth;
   }

   public float getScoreboardHealth(EntityLivingBase base) {
      try {
         ScoreObjective scoreBoard;
         if (base != null
            && base.getEntityWorld() != null
            && base.getEntityWorld().getScoreboard() != null
            && (scoreBoard = base.getEntityWorld().getScoreboard().getObjectiveInDisplaySlot(2)) != null) {
            return Float.parseFloat(String.valueOf(base.getEntityWorld().getScoreboard().getOrCreateScore(base.getName(), scoreBoard).getScorePoints()));
         }
   ...
   public boolean attackEntityFrom(DamageSource source, float amount) {
      if ((source.isFireDamage() && !this.isPotionActive(MobEffects.FIRE_RESISTANCE) || source.isMagicDamage() || source.isProjectile())
      ...
      if (this.isEntityInvulnerable(source)) {
         ...
         if (this.getHealth() <= 0.0F) {
            ...
            if (entity1 != null) {
               ...
               } else if (entity1 instanceof EntityWolf entitywolf && entitywolf.isTamed()) {
                  this.recentlyHit = 100;
                  this.attackingPlayer = null;
               }
            }
   ...
   private boolean func_190628_d(DamageSource p_190628_1_) {
      if (p_190628_1_.canHarmInCreative()) {
         ...
         if (itemstack != null) {
            if (this instanceof EntityPlayerMP entityplayermp) {
   ...
   public boolean isOnLadder() {
      ...
      int j = MathHelper.floor(this.getEntityBoundingBoxCLCompact().minY);
   ...
   public float getFallDistanceDamage(double distanceIn) {
      float damageAmount = 0.0F;
      if (!this.isEntityInvulnerable(DamageSource.fall)) {
         float damageMul = 1.0F;
         PotionEffect jumpBoost = this.getActivePotionEffect(MobEffects.JUMP_BOOST);
         if ((damageAmount = (float)MathHelper.ceil((distanceIn - 3.0 - (double)(jumpBoost == null ? 0 : jumpBoost.getAmplifier() + 1)) * (double)damageMul))
            != 0.0F) {
            damageAmount = this.applyArmorCalculations(DamageSource.fall, damageAmount);
            damageAmount = this.applyPotionDamageCalculations(DamageSource.fall, damageAmount);
         }
      }
   ...
   public boolean fallDistanceIsUnsafe(double distanceIn, float minHealth) {
      return this.getFallDistanceDamage(distanceIn) >= this.getHealth() + this.getAbsorptionAmount() - minHealth;
   }
   ...
   public final float getMaxHealth() {
      IAttributeInstance inst = this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH);
      if (inst == null) {
         return 20.0F;
      } else {
         float val = (float)this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).getAttributeValue();
         return val != 0.0F && val != 1000.0F ? val : 20.0F;
      }
   }
   ...
   private int getArmSwingAnimationEnd() {
      return ViewModel.getSwingSpeed(
         this, this.isPotionActive(MobEffects.MINING_FATIGUE) ? 6 + (1 + this.getActivePotionEffect(MobEffects.MINING_FATIGUE).getAmplifier()) * 2 : 6
      );
   }
         ...
         if (enumfacing1 != null) {
            ...
            for (int[] aint : aint1) {
               ...
               if (!this.world.collidesWithAnyBlock(axisalignedbb1)) {
                  ...
                  && this.world.getBlockState(new BlockPos(d11, this.posY + 1.0, d12)).isFullyOpaque()) {
                  d1 = d11;
                  d13 = this.posY + 2.0;
                  d14 = d12;
               }
   ...
   protected void jump() {
      boolean calledFlyHighJumpMatrix = HighJump.get != null && HighJump.get.callMatrixHighJump();
      HighJump highjump = HighJump.get;
      boolean customlLevel = this instanceof EntityPlayerSP
         && highjump.actived
         && highjump.JumpMode.currentMode.equalsIgnoreCase("Normal")
         && highjump.GroundJump.getBool();
      double boost = customlLevel ? (double)highjump.JumpPulse.getFloat() : (double)this.getJumpUpwardsMotion();
      if (boost == 0.42F
         && this instanceof EntityPlayerSP sp
         && HitAura.get.isActived()
         && HitAura.TARGET_ROTS != null
         && HitAura.get.CritsHelper.currentMode.equalsIgnoreCase("Grim")
         && sp.isCollidedVertically
         && !MoveMeHelp.moveKeysPressed()) {
         boost = 0.41F;
      }

      if (!calledFlyHighJumpMatrix) {
         this.motionY = boost;
      }

      if (!calledFlyHighJumpMatrix && this.isPotionActive(MobEffects.JUMP_BOOST)) {
         this.motionY = this.motionY + (double)((float)(this.getActivePotionEffect(MobEffects.JUMP_BOOST).getAmplifier() + 1) * 0.1F);
      }

      if (!calledFlyHighJumpMatrix && this.isSprinting()) {
         EventRotationJump e = new EventRotationJump(this.rotationYaw);
         if (this instanceof EntityPlayerSP) {
            e.call();
         }

         float f = (this instanceof EntityPlayerSP ? e.getYaw() : this.rotationYaw) * (float) (Math.PI / 180.0);
   ...
   protected void handleJumpWater() {
      this.motionY = this.motionY + (this.hasNewVersionMoves ? 0.04F : 0.04F);
   }
   ...
   protected float getWaterSlowDown() {
      return (float)WaterSpeed.getWaterSlowDownMultiply(this.isSprinting() && NewPhisicsFixes.isNewVersion() ? 0.9F : 0.8F, this);
   }
         ...
         if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying) {
            if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying) {
               if (this.isElytraFlying()) {
                  ...
                  float f = eventElytra.getPitch() * (float) (Math.PI / 180.0);
                  ...
                  if (this.motionY < 0.0 && d6 > 0.0) {
                     ...
                     this.motionY += d2;
                     this.motionX = this.motionX + vec3d.xCoord * d2 / d6;
                     this.motionZ = this.motionZ + vec3d.zCoord * d2 / d6;
                  }

                  if (f < 0.0F) {
                     double d10 = d8 * (double)(-MathHelper.sin(f)) * 0.04;
                     this.motionY += d10 * 3.2;
                     this.motionX = this.motionX - vec3d.xCoord * d10 / d6;
                     this.motionZ = this.motionZ - vec3d.zCoord * d10 / d6;
                  }

                  if (d6 > 0.0) {
                     this.motionX = this.motionX + (vec3d.xCoord / d6 * d8 - this.motionX) * 0.1;
                     this.motionZ = this.motionZ + (vec3d.zCoord / d6 * d8 - this.motionZ) * 0.1;
                  }
                  ...
                  BlockPos.PooledMutableBlockPos blockpos$pooledmutableblockpos = BlockPos.PooledMutableBlockPos.retain(
                     this.posX, this.getEntityBoundingBoxCLCompact().minY - 1.0, this.posZ
                  );
                  if (this.onGround) {
                  ...
                  if (this.onGround) {
                     f6 = this.world
                           .getBlockState(blockpos$pooledmutableblockpos.setPos(this.posX, this.getEntityBoundingBox().minY - 1.0, this.posZ))
                           .getBlock()
                           .slipperiness
                        * 0.91F;
                  }
                  ...
                  if (this.isPotionActive(MobEffects.LEVITATION)) {
                     this.motionY = this.motionY + (0.05 * (double)(this.getActivePotionEffect(MobEffects.LEVITATION).getAmplifier() + 1) - this.motionY) * 0.2;
                  } else {
               ...
               if (this.isCollidedHorizontally && this.isOffsetPositionInLiquid(this.motionX, this.motionY + 0.6F - this.posY + d4, this.motionZ)) {
                  this.motionY = 0.3F;
               }
            }
         } else {
            ...
            if (newVersion) {
               ...
               if (f3 > 0.0F) {
                  ...
                  f2 = (float)((double)f2 + (WaterSpeed.AIMoveWaterSpeedMultiply((double)this.getAIMoveSpeed(), this) - (double)f2) * (double)f3 / 3.0);
               }
            } else {
               if (f3 > 3.0F) {
                  f3 = 3.0F;
            ...
            this.moveEntity(MoverType.SELF, this.motionX, this.motionY, this.motionZ);
            boolean ladderFlag;
            if (ladderFlag = this.isCollidedHorizontally && this.isOnLadder()) {
               this.motionY = 0.2;
            }
            ...
            if (this.isCollidedHorizontally
               && this.isOffsetPositionInLiquid(this.motionX, this.motionY + (newVersion ? 0.6F : 0.6F) - this.posY + d0, this.motionZ)) {
               this.motionY = newVersion ? 0.3F : 0.3F;
            }
         }
      }
   ...
   public void onUpdate() {
      ...
      if (this.hurtTime <= 1) {
         this.preHurtHealth = this.getHealth();
         float u = this.preHurtHealth;
         this.smoothPreHurtHealth.to = u;
      }

      this.smoothPreHurtHealth.speed = 0.1F;
      if (this.hasNewVersionMoves = NewPhisicsFixes.isNewVersion() && this instanceof EntityPlayer) {
      ...
      if (!this.world.isRemote) {
         ...
         for (EntityEquipmentSlot entityequipmentslot : EntityEquipmentSlot.values()) {
            ...
            if (!ItemStack.areItemStacksEqual(itemstack1, itemstack)) {
               ((WorldServer)this.world)
                  .getEntityTracker()
                  .sendToAllTrackingEntity(this, new SPacketEntityEquipment(this.getEntityId(), entityequipmentslot, itemstack1));
               if (!itemstack.func_190926_b()) {
                  this.getAttributeMap().removeAttributeModifiers(itemstack.getAttributeModifiers(entityequipmentslot));
               }
      ...
      f5 = this.updateDistance(f4, f5, this.rotationYaw);
      this.world.theProfiler.endSection();
   ...
   public float updateDistance(float p_110146_1_, float p_110146_2_, float virtYaw) {
   ...
   public void onLivingUpdate() {
      ...
      if (this.newPosRotationIncrements > 0 && !this.canPassengerSteer()) {
         ...
         this.rotationPitch = (float)(
            (double)this.rotationPitch + (this.interpTargetPitch - (double)this.rotationPitch) / (double)this.newPosRotationIncrements
         );
      ...
      this.updateElytra();
      if (Panic.stop || !(this instanceof EntityPlayerSP) || !AirStuck.ifStopMotionOrder()) {
   ...
   private void updateElytra() {
      boolean flag = this.isElytraFlying();
   ...
   protected void collideWithNearbyEntities() {
      List<Entity> list = this.world.getEntitiesInAABBexcluding(this, this.getEntityBoundingBoxCLCompact(), EntitySelectors.getTeamCollisionPredicate(this));
      if (!list.isEmpty()) {
   ...
   public Vec3d getLook(float partialTicks) {
      if (partialTicks == 1.0F) {
         return getVectorForRotation(this.rotationPitchHead, this.rotationYawHead);
      } else {
   ...
   public float getAbsorptionAmount() {
      return (int)this.absorptionAmount == 1000 ? 0.0F : this.absorptionAmount;
   }
   ...
   protected void updateItemUse(ItemStack stack, int eatingParticleCount) {
      if (!stack.func_190926_b() && this.isHandActive()) {
         if (stack.getItemUseAction() == EnumAction.EAT) {
            if (!NoRender.get.actived || !NoRender.get.EatParticles.getBool()) {
            ...
            this.playSound(
               SoundEvents.ENTITY_GENERIC_EAT, 0.5F + 0.5F * (float)this.rand.nextInt(2), (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F
            );
   ...
   public boolean isActiveItemStackBlocking() {
      if (this.isHandActive() && !this.activeItemStack.func_190926_b()) {
         ...
         return item.getItemUseAction(this.activeItemStack) != EnumAction.BLOCK
            ? false
            : item.getMaxItemUseDuration(this.activeItemStack) - this.activeItemStackUseCount >= 5;
      } else {
         return false;
      }
   }
   ...
   public boolean isElytraFlying() {
      return this.getFlag(7) && (!Bypass.get.isActived() || !Bypass.get.ncpDisabledLocalBool);
   }
   ...
   public boolean attemptTeleport(double x, double y, double z) {
      ...
      if (world.isBlockLoaded(blockpos)) {
         ...
         if (flag1) {
            ...
            if (world.getCollisionBoxes(this, this.getEntityBoundingBoxCLCompact()).isEmpty() && !world.containsAnyLiquid(this.getEntityBoundingBoxCLCompact())
               )
             {
               flag = true;
            }
   ...
   public EntityLivingBase cloneEntity() {
      return (EntityLivingBase)this.clone();
   }
   ...
   protected Object clone() {
      ...
      return clone instanceof EntityLivingBase base ? base : null;
   }
}
