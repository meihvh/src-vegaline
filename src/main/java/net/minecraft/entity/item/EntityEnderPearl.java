...
public class EntityEnderPearl extends EntityThrowable {
   ...
   public EntityLivingBase parsedThrower;
   ...
   public EntityPlayer getFirstNeareblePlayerAsThrower() {
      if (Minecraft.getMinecraft().world != null) {
         List<EntityPlayer> players = Minecraft.getMinecraft()
            .world
            .playerEntities
            .stream()
            .map(player1 -> player1 instanceof EntityOtherPlayerMP mp ? mp : null)
            .filter(Objects::nonNull)
            .filter(mp -> mp.getDistanceAtEye(this.posX, this.posY, this.posZ) < 2.0)
            .collect(Collectors.toList());
         if (players.size() > 1) {
            players = players.stream().sorted(Comparator.comparingDouble(obj -> (double)(-obj.getDistanceToEntity(this)))).collect(Collectors.toList());
         }
   ...
   protected void onImpact(RayTraceResult result) {
      ...
      if (result.typeOfHit == RayTraceResult.Type.BLOCK) {
         ...
         if (this.world.getTileEntity(blockpos) instanceof TileEntityEndGateway tileentityendgateway) {
   ...
   public void setFirstNeareblePlayer() {
      if (this.parsedThrower == null) {
         EntityPlayer otherNeareble = this.getFirstNeareblePlayerAsThrower();
         if (otherNeareble != null) {
            this.parsedThrower = otherNeareble;
         }
      }
   }
