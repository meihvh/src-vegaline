...
public class EntityFireworkRocket extends Entity {
   ...
   public void setVelocity(double x, double y, double z) {
      ...
      if (this.prevRotationPitch == 0.0F && this.prevRotationYaw == 0.0F) {
         ...
         this.rotationPitch = (float)(MathHelper.atan2(y, (double)f) * (180.0 / Math.PI));
   ...
   public void onUpdate() {
      ...
      if (this.func_191511_j()) {
         if (this.field_191513_e == null) {
            Entity entity = this.world.getEntityByID(this.dataManager.get(field_191512_b));
         ...
         if (this.field_191513_e != null) {
            if (this.field_191513_e.isElytraFlying()) {
               ...
               if (this.field_191513_e instanceof EntityPlayerSP) {
                  EventElytraVector event = (EventElytraVector)new EventElytraVector(
                        vec3d, this.field_191513_e, this.field_191513_e.rotationYaw, this.field_191513_e.rotationPitch
                     )
                     .call();
                  event.setVectorAsYawPitch(event.getYaw(), event.getPitch());
                  vec3d = event.getVector();
               }

               double d0 = 1.5;
               double d1 = 0.1;
               this.field_191513_e.motionX = this.field_191513_e.motionX + vec3d.xCoord * 0.1 + (vec3d.xCoord * 1.5 - this.field_191513_e.motionX) * 0.5;
               this.field_191513_e.motionY = this.field_191513_e.motionY + vec3d.yCoord * 0.1 + (vec3d.yCoord * 1.5 - this.field_191513_e.motionY) * 0.5;
               this.field_191513_e.motionZ = this.field_191513_e.motionZ + vec3d.zCoord * 0.1 + (vec3d.zCoord * 1.5 - this.field_191513_e.motionZ) * 0.5;
            }
            ...
            if (entitylivingbase != this.field_191513_e && this.getDistanceSqToEntity(entitylivingbase) <= 25.0) {
               ...
               for (int i = 0; i < 2; i++) {
                  RayTraceResult raytraceresult = this.world
                     .rayTraceBlocks(
                        vec3d,
                        new Vec3d(entitylivingbase.posX, entitylivingbase.posY + (double)entitylivingbase.height * 0.5 * (double)i, entitylivingbase.posZ),
                        false,
                        true,
                        false
                     );
