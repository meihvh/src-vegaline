      ...
      protected void func_190868_j() {
         ...
         if (EntityEvoker.this.getDistanceSqToEntity(entitylivingbase) < 9.0) {
            for (int i = 0; i < 5; i++) {
               ...
               this.func_190876_a(
                  EntityEvoker.this.posX + (double)MathHelper.cos(f1) * 1.5, EntityEvoker.this.posZ + (double)MathHelper.sin(f1) * 1.5, d0, d1, f1, 0
               );
            }
            ...
            for (int k = 0; k < 8; k++) {
               ...
               this.func_190876_a(
                  EntityEvoker.this.posX + (double)MathHelper.cos(f2) * 2.5, EntityEvoker.this.posZ + (double)MathHelper.sin(f2) * 2.5, d0, d1, f2, 3
               );
            }
         } else {
            for (int l = 0; l < 16; l++) {
               ...
               int j = 1 * l;
               this.func_190876_a(
                  EntityEvoker.this.posX + (double)MathHelper.cos(f) * d2, EntityEvoker.this.posZ + (double)MathHelper.sin(f) * d2, d0, d1, f, j
               );
            }
         }
      }
         ...
         if (EntityEvoker.this.getAttackTarget() != null) {
            EntityEvoker.this.getLookHelper()
               .setLookPositionWithEntity(
                  EntityEvoker.this.getAttackTarget(), (float)EntityEvoker.this.getHorizontalFaceSpeed(), (float)EntityEvoker.this.getVerticalFaceSpeed()
               );
         } else if (EntityEvoker.this.func_190751_dj() != null) {
            EntityEvoker.this.getLookHelper()
               .setLookPositionWithEntity(
                  EntityEvoker.this.func_190751_dj(), (float)EntityEvoker.this.getHorizontalFaceSpeed(), (float)EntityEvoker.this.getVerticalFaceSpeed()
               );
         }
      }
   }
      ...
      public boolean shouldExecute() {
         if (EntityEvoker.this.getAttackTarget() != null) {
            ...
            List<EntitySheep> list = EntityEvoker.this.world
               .getEntitiesWithinAABB(EntitySheep.class, EntityEvoker.this.getEntityBoundingBox().expand(16.0, 4.0, 16.0), this.field_190879_a);
            if (list.isEmpty()) {
               return false;
            } else {
