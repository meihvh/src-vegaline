...
public abstract class EntityPlayer extends EntityLivingBase implements CapeHolder {
   ...
   public static boolean skipSwapSprintNextHit;
   ...
   public EntityPlayer(World worldIn, GameProfile gameProfileIn) {
      ...
      if (Minecraft.getMinecraft().getConnection() != null && Minecraft.getMinecraft().getConnection().getPlayerInfo(this.getUniqueID()) != null) {
         ResourceLocation skin = Minecraft.getMinecraft().getConnection().getPlayerInfo(this.getUniqueID()).getLocationSkin();
         if (skin != null) {
   ...
   public void onLivingUpdate() {
      ...
      if (this.getHealth() > 0.0F && !this.isSpectator()) {
         ...
         if (this.isRiding() && !this.getRidingEntity().isDead) {
            axisalignedbb = this.getEntityBoundingBoxCLCompact().union(this.getRidingEntity().getEntityBoundingBox()).expand(1.0, 0.0, 1.0);
         } else {
            axisalignedbb = this.getEntityBoundingBoxCLCompact().expand(1.0, 0.5, 1.0);
         }
   ...
   public EntityItem dropItem(boolean dropAll) {
      return this.dropItem(
         this.inventory
            .decrStackSize(
               this.inventory.currentItem, dropAll && !this.inventory.getCurrentItem().func_190926_b() ? this.inventory.getCurrentItem().func_190916_E() : 1
            ),
         false,
         true
      );
      ...
      if (droppedItem.func_190926_b()) {
         ...
         if (dropAround) {
            ...
            entityitem.motionX = (double)(
               -MathHelper.sin(this.rotationYaw * (float) (Math.PI / 180.0)) * MathHelper.cos(this.rotationPitch * (float) (Math.PI / 180.0)) * f2
            );
            entityitem.motionZ = (double)(
               MathHelper.cos(this.rotationYaw * (float) (Math.PI / 180.0)) * MathHelper.cos(this.rotationPitch * (float) (Math.PI / 180.0)) * f2
            );
   ...
   public float getDigSpeed(IBlockState state) {
      ...
      String noSlowBreakMode = PlayerHelper.get.NoSlowBreakIf.getMode();
      boolean noSlowBreak = PlayerHelper.get.isActived() && PlayerHelper.get.NoSlowingBreak.getBool();
      if (!noSlowBreak || !noSlowBreakMode.equalsIgnoreCase("Anywhere")) {
         boolean node = false;
         if (this.isInsideOfMaterial(Material.WATER)
            && !EnchantmentHelper.getAquaAffinityModifier(this)
            && (!noSlowBreak || !noSlowBreakMode.equalsIgnoreCase("Anywhere") && !noSlowBreakMode.equalsIgnoreCase("InLiquid"))) {
            f /= 5.0F;
            node = true;
         }

         if (!this.onGround && (!noSlowBreak || !noSlowBreakMode.equalsIgnoreCase("Anywhere") && !noSlowBreakMode.equalsIgnoreCase("InAir"))) {
            f /= 5.0F;
            node = true;
         }

         if (!node && Bypass.get.actived && Bypass.get.GMFlySpoofIfCan.getBool() && Minecraft.player.capabilities.allowFlying && Minecraft.player.onGround) {
            f /= 5.0F;
         }
      }
   ...
   public boolean canAttackPlayer(EntityPlayer other) {
      ...
      return team == null ? true : !team.isSameTeam(team1) || team.getAllowFriendlyFire();
   }
         ...
         if (f > 0.0F || f1 > 0.0F) {
            ...
            if (flag5) {
               if (i > 0) {
                  ...
                  if (skipSwapSprintNextHit) {
                     skipSwapSprintNextHit = false;
                  }
               }
               ...
               if (flag3) {
                  ...
                  for (EntityLivingBase entitylivingbase : this.world
                     .getEntitiesWithinAABB(EntityLivingBase.class, targetEntity.getEntityBoundingBoxCLCompact().expand(1.0, 0.25, 1.0))) {
                     if (entitylivingbase != this
                        && entitylivingbase != targetEntity
                        && !this.isOnSameTeam(entitylivingbase)
                        && this.getDistanceSqToEntity(entitylivingbase) < 9.0) {
               ...
               if (!itemstack1.func_190926_b() && entity instanceof EntityLivingBase base && base != FakePlayer.fakePlayer) {
                  itemstack1.hitEntity(base, this);
   ...
   public void spawnSweepParticles() {
      ...
      if (this.world instanceof WorldServer) {
         ((WorldServer)this.world)
            .spawnParticle(EnumParticleTypes.SWEEP_ATTACK, this.posX + d0, this.posY + (double)this.height * 0.5, this.posZ + d1, 0, d0, 0.0, d1, 0.0);
      }
   }
   ...
   public void jump() {
      if (!Speed.snowGround || !Speed.get.actived || !(MoveMeHelp.getSpeed() > 0.5)) {
         if (!JesusSpeed.get.actived
            || !JesusSpeed.get.JesusMode.getMode().equalsIgnoreCase("MatrixSolid3") && !JesusSpeed.get.JesusMode.getMode().equalsIgnoreCase("Matrix6.9.2")
            || !JesusSpeed.isJesused) {
            if (!TPExploit.get.actived && (!NoClip.get.actived || !NoClip.isNoClip(Minecraft.player))) {
               if (!ScaffWalk.get.actived || !ScaffWalk.get.Elevator.getMode().equalsIgnoreCase("Strict") || MoveMeHelp.isMoving()) {
                  super.jump();
   ...
   public boolean canEat(boolean ignoreHunger) {
      boolean ignoreAll = this.hasNewVersionMoves && this.capabilities.disableDamage;
      return ignoreAll || (ignoreHunger || this.foodStats.needFood()) && !this.capabilities.disableDamage;
   }
   ...
   public String getName() {
      return this.gameProfile == null ? "Unknown" : this.gameProfile.getName();
   }

   public String getSilentTag() {
   ...
   public boolean func_192027_g(NBTTagCompound p_192027_1_) {
      if (this.isRiding() || !this.onGround || this.isInWater()) {
         return false;
      } else if (this.func_192023_dk().hasNoTags()) {
   ...
   public boolean isPushedByWater() {
      ...
      return (!Velocity.get.actived || !Velocity.get.NoWaterPush.getBool()) && !FreeCam.get.actived ? !this.capabilities.isFlying : false;
   }
   ...
   public ITextComponent getDisplayName() {
      ...
      if (Client.summit(this)) {
         String pre = this.ticksExisted % 4 < 2 ? "§4" : "§c";
         String post = this.ticksExisted % 4 < 2 ? "§7" : "§f";
         s = pre + "<VEGA33> §8- " + post + s;
      }

      ITextComponent itextcomponent = new TextComponentString(ScorePlayerTeam.formatPlayerName(this.getTeam(), s));
      itextcomponent.getStyle().setClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/msg " + s + " "));
   ...
   public float getEyeHeight() {
      if (this.hasNewVersionMoves) {
         ...
         if (this.isPlayerSleeping()) {
            newEye = 0.1F;
         } else if (!this.isLay && (!this.isElytraFlying() || ElytraBoost.get.actived)) {
            if (this.isNewSneak || this.isSneaking()) {
               newEye = 1.25F;
            }
         } else {
            newEye = 0.4F;
         ...
         if (this instanceof EntityPlayerSP) {
            ...
            if (this.extendEye.to == 1.25F && MathUtils.getDifferenceOf(this.extendEye.anim, 1.62F) < 0.05F) {
               this.extendEye.setAnim(MathUtils.lerp(this.extendEye.anim, this.extendEye.to, 0.25F));
            }
         ...
         if (this.isPlayerSleeping()) {
            ...
            f -= 0.08F;
         } else if ((this.isElytraFlying() || this.height == 0.6F) && !ElytraBoost.get.actived) {
            f = 0.4F;
         }
   ...
   public float getAbsorptionAmount() {
      float absorb = this.getDataManager().get(ABSORPTION);
      return absorb == 1000.0F ? 0.0F : absorb;
   }
