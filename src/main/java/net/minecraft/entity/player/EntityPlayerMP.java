         ...
         if (!flag && this.respawnInvulnerabilityTicks > 0 && source != DamageSource.outOfWorld) {
            ...
            if (source instanceof EntityDamageSource) {
               ...
               if (entity instanceof EntityArrow entityarrow
                  && entityarrow.shootingEntity instanceof EntityPlayer
   ...
   public void displayGUIChest(IInventory chestInventory) {
      if (chestInventory instanceof ILootContainer && ((ILootContainer)chestInventory).getLootTable() != null && this.isSpectator()) {
         ...
         if (chestInventory instanceof ILockableContainer ilockablecontainer
            && ilockablecontainer.isLocked()
            && !this.canOpen(ilockablecontainer.getLockCode())
            && !this.isSpectator()) {
            this.connection
         ...
         if (chestInventory instanceof IInteractionObject) {
            ...
            this.connection
               .sendPacket(
                  new SPacketOpenWindow(this.currentWindowId, "minecraft:container", chestInventory.getDisplayName(), chestInventory.getSizeInventory())
               );
