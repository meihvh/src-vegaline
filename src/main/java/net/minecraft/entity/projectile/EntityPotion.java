...
public class EntityPotion extends EntityThrowable {
   ...
   protected void onImpact(RayTraceResult result) {
      if (!NameTags.canCancelHandleWorldPotionEffects(this.world.isRemote)) {
         ItemStack itemstack = this.getPotion();
         PotionType potiontype = PotionUtils.getPotionFromItem(itemstack);
         List<PotionEffect> list = PotionUtils.getEffectsFromStack(itemstack);
         boolean flag = potiontype == PotionTypes.WATER && list.isEmpty();
         if (!this.world.isRemote && result.typeOfHit == RayTraceResult.Type.BLOCK && flag) {
   ...
   private void func_190543_a(RayTraceResult p_190543_1_, List<PotionEffect> p_190543_2_) {
      ...
      if (!list.isEmpty()) {
         for (EntityLivingBase entitylivingbase : list) {
            if ((!this.world.isRemote || !(entitylivingbase instanceof EntityPlayerSP)) && entitylivingbase.canBeHitWithPotion()) {
               double d0 = this.getDistanceSqToEntity(entitylivingbase);
               if (d0 < 16.0) {
                     ...
                     if (potion.isInstant()) {
                        ...
                        if (i > 20) {
                           entitylivingbase.addPotionEffect(
                              new PotionEffect(potion, i, potioneffect.getAmplifier(), potioneffect.getIsAmbient(), potioneffect.doesShowParticles())
                           );
                        }
                     }
                  }
