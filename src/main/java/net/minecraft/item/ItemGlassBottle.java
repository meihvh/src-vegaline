...
public class ItemGlassBottle extends Item {
   ...
   public ActionResult<ItemStack> onItemRightClick(World itemStackIn, EntityPlayer worldIn, EnumHand playerIn) {
      List<EntityAreaEffectCloud> list = itemStackIn.getEntitiesWithinAABB(
         EntityAreaEffectCloud.class, worldIn.getEntityBoundingBox().expandXyz(2.0), new Predicate<EntityAreaEffectCloud>() {
      ...
      if (!list.isEmpty()) {
         ...
         itemStackIn.playSound(
            (EntityPlayer)null, worldIn.posX, worldIn.posY, worldIn.posZ, SoundEvents.ITEM_BOTTLE_FILL_DRAGONBREATH, SoundCategory.NEUTRAL, 1.0F, 1.0F
         );
         ...
         if (raytraceresult == null) {
            ...
            if (raytraceresult.typeOfHit == RayTraceResult.Type.BLOCK) {
               ...
               if (!itemStackIn.isBlockModifiable(worldIn, blockpos)
                  || !worldIn.canPlayerEdit(blockpos.offset(raytraceresult.sideHit), raytraceresult.sideHit, itemstack)) {
                  return new ActionResult<>(EnumActionResult.PASS, itemstack);
               }
               ...
               if (itemStackIn.getBlockState(blockpos).getMaterial() == Material.WATER) {
                  ...
                  return new ActionResult<>(
                     ...
                     this.turnBottleIntoItem(itemstack, worldIn, PotionUtils.addPotionToItemStack(new ItemStack(Items.POTIONITEM), PotionTypes.WATER))
                  );
               }
            }
