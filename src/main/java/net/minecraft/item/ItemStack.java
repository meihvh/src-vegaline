...
public final class ItemStack {
   ...
   public boolean anyString(String string, boolean lowerContain) {
      return this.getTooltip(Minecraft.player, ITooltipFlag.TooltipFlags.ADVANCED)
         .stream()
         .anyMatch(toolStr -> (lowerContain ? toolStr.toLowerCase() : toolStr).contains(string));
   }

   public boolean anyDoubleString(String string, String string2, boolean lowerContain) {
      return this.getTooltip(Minecraft.player, ITooltipFlag.TooltipFlags.ADVANCED)
         .stream()
         .anyMatch(
            toolStr -> (lowerContain ? toolStr.toLowerCase() : toolStr).contains(string) && (lowerContain ? toolStr.toLowerCase() : toolStr).contains(string2)
         );
   }
   ...
   boolean unlimit() {
      return !Panic.stop
         && this.mc().playerController != null
         && this.mc().playerController.getCurrentGameType() == GameType.CREATIVE
         && ProContainer.get.actived
         && ProContainer.get.StackUnlimit.getBool();
   }
   ...
   public int getMaxDamage() {
      if (this.getItem() instanceof ItemArmor armor && armor.getArmorMaterial() == ItemArmor.ArmorMaterial.NETHERITE) {
         return new int[]{481, 555, 592, 407}[armor.armorType.getIndex()];
      }
   ...
   public void damageItem(int amount, EntityLivingBase entityIn) {
      if ((!(entityIn instanceof EntityPlayer) || !((EntityPlayer)entityIn).capabilities.isCreativeMode)
         ...
         && this.attemptDamageItem(amount, entityIn.getRNG(), entityIn instanceof EntityPlayerMP ? (EntityPlayerMP)entityIn : null)) {
   ...
   public static boolean areItemStacksEqual(ItemStack stackA, ItemStack stackB) {
      return stackA.func_190926_b() && stackB.func_190926_b() ? true : !stackA.func_190926_b() && !stackB.func_190926_b() && stackA.isItemStackEqual(stackB);
   }
   ...
   public float getItemAttackDamage(ItemStack item) {
      return (float)item.itemDamage;
   }

   public static boolean areItemsEqual(ItemStack stackA, ItemStack stackB) {
      return stackA == stackB ? true : !stackA.func_190926_b() && !stackB.func_190926_b() && stackA.isItemEqual(stackB);
   }

   public static boolean areItemsEqualIgnoreDurability(ItemStack stackA, ItemStack stackB) {
      return stackA == stackB ? true : !stackA.func_190926_b() && !stackB.func_190926_b() && stackA.isItemEqualIgnoreDurability(stackB);
   }
   ...
   public List<String> getTooltip(@Nullable EntityPlayer playerIn, ITooltipFlag advanced) {
      ...
      if (advanced.func_194127_a()) {
         if (this.isItemDamaged()) {
            ...
            if (this.getItem() instanceof ItemArmor armor && armor.getArmorMaterial() == ItemArmor.ArmorMaterial.NETHERITE) {
               max = new int[]{481, 555, 592, 407}[armor.armorType.getIndex()];
            }

            list.add(I18n.translateToLocalFormatted("item.durability", max - this.getItemDamage(), max));
         }
      ...
      if (this.hasTagCompound() && this.stackTagCompound.hasKey("AttributeModifiers", 9)) {
         ...
         for (int i = 0; i < nbttaglist.tagCount(); i++) {
            ...
            if (attributemodifier != null
               && (!nbttagcompound.hasKey("Slot", 8) || nbttagcompound.getString("Slot").equals(equipmentSlot.getName()))
