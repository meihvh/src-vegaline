...
public abstract class TileEntity {
   ...
   private int glowFramesCounter;
   private boolean glowing = false;
   private int colorGlowing = -1;
   private ScopeTileEntityModelTo2D scope2d = new ScopeTileEntityModelTo2D(this);
   protected Block blockType;

   public ScopeTileEntityModelTo2D getScope2d() {
      if (this.scope2d == null) {
         this.scope2d = new ScopeTileEntityModelTo2D(this);
      }

      return this.scope2d;
   }

   public void setGlowing(boolean glowing, int framesCount) {
      this.glowing = glowing;
      this.glowFramesCounter = framesCount;
   }

   public void setGlowingPostGlowFrames(boolean glowing, int framesCount) {
      if (glowing) {
         this.glowing = true;
         this.glowFramesCounter = framesCount;
      } else if (this.glowing) {
         this.glowFramesCounter--;
         if (this.glowFramesCounter <= 0) {
            this.glowing = false;
         }
      }
   }
   ...
   public void setColorGlowing(int color) {
      this.colorGlowing = color;
   }

   public int getColorGlowing() {
   ...
   public double getDistanceToTileEntity(TileEntity tileEntityIn) {
      ...
      return Math.sqrt((double)(f * f + f1 * f1 + f2 * f2));
   }
   ...
   private NBTTagCompound writeInternal(NBTTagCompound compound) {
      ResourceLocation resourcelocation = field_190562_f.getNameForObject((Class<? extends TileEntity>)this.getClass());
      ...
      reportCategory.setDetail(
         ...
         new ICrashReportDetail<String>() {
            public String call() throws Exception {
               return TileEntity.field_190562_f.getNameForObject((Class<? extends TileEntity>)TileEntity.this.getClass())
                  + " // "
