...
public class TileEntityEndGateway extends TileEntityEndPortal implements ITickable {
   ...
   private void findExitPortal() {
      ...
      for (int i = 16; getChunk(this.world, vec3d1).getTopFilledSegment() > 0 && i-- > 0; vec3d1 = vec3d1.add(vec3d.scale(-16.0))) {
         LOG.debug("Skipping backwards past nonempty chunk at {}", vec3d1);
      }

      for (int j = 16; getChunk(this.world, vec3d1).getTopFilledSegment() == 0 && j-- > 0; vec3d1 = vec3d1.add(vec3d.scale(16.0))) {
   ...
   private void createExitPortal(BlockPos posIn) {
      ...
      if (this.world.getTileEntity(posIn) instanceof TileEntityEndGateway tileentityendgateway) {
