...
public class TileEntityHopper extends TileEntityLockableLoot implements IHopper, ITickable {
   ...
   public ItemStack decrStackSize(int index, int count) {
      ...
      return ItemStackHelper.getAndSplit(this.func_190576_q(), index, count);
   }
   ...
   private static ItemStack insertStack(IInventory inventoryIn, IInventory stack, ItemStack index, int side, EnumFacing p_174916_4_) {
      ...
      if (canInsertItemInSlot(stack, index, side, p_174916_4_)) {
         ...
         if (flag) {
            if (flag1 && stack instanceof TileEntityHopper tileentityhopper1 && !tileentityhopper1.mayTransfer()) {
               int k = 0;
               if (inventoryIn != null
                  && inventoryIn instanceof TileEntityHopper tileentityhopper
                  && tileentityhopper1.field_190578_g >= tileentityhopper.field_190578_g) {
                  k = 1;
               }
   ...
   public static IInventory getInventoryAtPosition(World worldIn, double x, double y, double z) {
      ...
      if (iinventory == null) {
         List<Entity> list = worldIn.getEntitiesInAABBexcluding(
            (Entity)null, new AxisAlignedBB(x - 0.5, y - 0.5, z - 0.5, x + 0.5, y + 0.5, z + 0.5), EntitySelectors.HAS_INVENTORY
         );
         if (!list.isEmpty()) {
