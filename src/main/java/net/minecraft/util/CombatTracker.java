   ...
   public void calculateFallSuffix() {
      ...
      if (this.fighter.isOnLadder()) {
         Block block = this.fighter
            .world
            .getBlockState(new BlockPos(this.fighter.posX, this.fighter.getEntityBoundingBoxCLCompact().minY, this.fighter.posZ))
            .getBlock();
   ...
   public ITextComponent getDeathMessage() {
      if (this.combatEntries.isEmpty()) {
         ...
         if (combatentry != null && combatentry1.getDamageSrc() == DamageSource.fall) {
            ...
            if (combatentry.getDamageSrc() == DamageSource.fall || combatentry.getDamageSrc() == DamageSource.outOfWorld) {
            ...
            } else if (itextcomponent2 != null && !itextcomponent2.equals(itextcomponent1)) {
               ...
               if (!itemstack1.func_190926_b() && itemstack1.hasDisplayName()) {
                  itextcomponent = new TextComponentTranslation(
                     "death.fell.assist.item", this.fighter.getDisplayName(), itextcomponent2, itemstack1.getTextComponent()
                  );
               } else {
               ...
               if (!itemstack.func_190926_b() && itemstack.hasDisplayName()) {
                  itextcomponent = new TextComponentTranslation(
                     "death.fell.finish.item", this.fighter.getDisplayName(), itextcomponent1, itemstack.getTextComponent()
                  );
               } else {
