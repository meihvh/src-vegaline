               ...
               public boolean apply(@Nullable Entity p_apply_1_) {
                  if (!p_apply_1_.canBePushed()) {
                     ...
                     if (team$collisionrule1 == Team.CollisionRule.NEVER) {
                        ...
                        return (team$collisionrule == Team.CollisionRule.HIDE_FOR_OWN_TEAM || team$collisionrule1 == Team.CollisionRule.HIDE_FOR_OWN_TEAM)
                              && flag
                           ? false
                           : team$collisionrule != Team.CollisionRule.HIDE_FOR_OTHER_TEAMS && team$collisionrule1 != Team.CollisionRule.HIDE_FOR_OTHER_TEAMS
                              || flag;
                     }
                  } else {
                     return false;
   ...
   public static class ArmoredMob implements Predicate<Entity> {
      ...
      public boolean apply(@Nullable Entity p_apply_1_) {
         if (!p_apply_1_.isEntityAlive()) {
            ...
            return false;
         } else if (!entitylivingbase.getItemStackFromSlot(EntityLiving.getSlotForItemStack(this.armor)).func_190926_b()) {
            return false;
