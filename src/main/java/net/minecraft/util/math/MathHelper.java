...
public class MathHelper {
   ...
   public static float sin(float value) {
      return fastMath
         ? SIN_TABLE_FAST[((int)(value * 651.8986F) & 4095) % SIN_TABLE_FAST.length]
         : SIN_TABLE[((int)(value * 10430.378F) & 65535) % SIN_TABLE.length];
   }

   public static float cos(float value) {
      return fastMath
         ? SIN_TABLE_FAST[((int)((value + (float) (Math.PI / 2)) * 651.8986F) & 4095) % SIN_TABLE_FAST.length]
         : SIN_TABLE[((int)(value * 10430.378F + 16384.0F) & 65535) % SIN_TABLE.length];
   }
   ...
   public static double easeInOutQuad(double x, int step) {
      return x < 0.5 ? 2.0 * x * x : 1.0 - Math.pow(-2.0 * x + 2.0, (double)step) / 2.0;
   }
   ...
   static {
      ...
      for (int k = 0; k < 360; k += 90) {
         SIN_TABLE_FAST[(int)((float)k * 11.377778F) & 4095] = (float)Math.sin((double)((float)k * (float) (Math.PI / 180.0)));
      }
