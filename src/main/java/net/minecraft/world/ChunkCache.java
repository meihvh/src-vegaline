...
public class ChunkCache implements IBlockAccess {
   ...
   public Biome getBiome(BlockPos pos) {
      int i = (pos.getX() >> 4) - this.chunkX;
      int j = (pos.getZ() >> 4) - this.chunkZ;
      return this.chunkArray[Math.max(i, 0)][Math.max(j, 0)].getBiome(pos, this.worldObj.getBiomeProvider());
   }
   ...
   private int getLightForExt(EnumSkyBlock type, BlockPos pos) {
      if (type == EnumSkyBlock.SKY && !this.worldObj.provider.func_191066_m()) {
         ...
         return type.defaultLightValue > 0 ? type.defaultLightValue : 0;
   ...
   public int getLightFor(EnumSkyBlock p_175628_1_, BlockPos pos) {
      if (pos.getY() >= 0 && pos.getY() < 256) {
         int i = (pos.getX() >> 4) - this.chunkX;
         int j = (pos.getZ() >> 4) - this.chunkZ;
         return this.chunkArray[i] == null ? p_175628_1_.defaultLightValue : this.chunkArray[i][j].getLightFor(p_175628_1_, pos);
      } else {
