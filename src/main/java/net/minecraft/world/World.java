...
public abstract class World implements IBlockAccess {
                  ...
                  public String call() throws Exception {
                     try {
                        return String.format(
                           "ID #%d (%s // %s)", Block.getIdFromBlock(p_190524_2_), p_190524_2_.getUnlocalizedName(), p_190524_2_.getClass().getCanonicalName()
                        );
                     ...
                     public String call() throws Exception {
                        try {
                           return String.format(
                              ...
                              p_190529_2_.getClass().getCanonicalName()
                           );
                        } catch (Throwable var2) {
   ...
   public int getLight(BlockPos pos, boolean checkNeighbors) {
      int scaleW = this.getWorldBorder() == null ? 30000000 : this.getWorldBorder().getSize();
   ...
   public RayTraceResult rayTraceBlocks(
      ...
      if (Double.isNaN(vec31.xCoord) || Double.isNaN(vec31.yCoord) || Double.isNaN(vec31.zCoord)) {
         ...
         Block block = iblockstate.getBlock();
         if ((!ignoreBlockWithoutBoundingBox || iblockstate.getCollisionBoundingBox(this, blockpos) != Block.NULL_AABB)
   ...
   public void playSound(@Nullable EntityPlayer player, double x, double y, double z, SoundEvent soundIn, SoundCategory category, float volume, float pitch) {
      if (ClientTune.get.getRTSoundSurround() && !ClientTune.get.getRTPerfomanceMode()) {
         Vec3d posBlockCenter = new Vec3d(x, y, z);
         if (player == null) {
         ...
         if (player != null && this != null) {
            ...
            Vec3d playerPos = new Vec3d(
               player.lastTickPosX + (player.posX - player.lastTickPosX) * (double)pt,
               player.lastTickPosY + (player.posY - player.lastTickPosY) * (double)pt,
               player.lastTickPosZ + (player.posZ - player.lastTickPosZ) * (double)pt
            );
            if (playerPos.distanceTo(posBlockCenter) <= 0.75) {
               float f = player.prevRotationPitch + (player.rotationPitch - player.prevRotationPitch) * pt;
               float f1 = player.prevRotationYaw + (player.rotationYaw - player.prevRotationYaw) * pt;
               x = RenderManager.viewerPosX;
               y = RenderManager.viewerPosY + (double)player.getEyeHeight();
               z = RenderManager.viewerPosZ;
               f += WorldRender.get.offPitchOrient;
               f1 += WorldRender.get.offYawOrient;
               float offset = -0.3F;
               x += (double)(MathHelper.sin(MathHelper.toRadians(f1)) * offset);
               y += (double)(f / 90.0F * offset);
               z += (double)(-MathHelper.cos(MathHelper.toRadians(f1)) * offset);
            }
         }
      }
   ...
   public void spawnParticle(
      ...
      if (!NoRender.get.isActived() || !NoRender.get.AllParticles.getBool()) {
   ...
   public void func_190523_a(
      ...
      if (!NoRender.get.isActived() || !NoRender.get.AllParticles.getBool()) {
         for (int i = 0; i < this.eventListeners.size(); i++) {
            this.eventListeners
               .get(i)
               .func_190570_a(p_190523_1_, false, true, p_190523_2_, p_190523_4_, p_190523_6_, p_190523_8_, p_190523_10_, p_190523_12_, p_190523_14_);
         }
      }
   }
   ...
   public void spawnParticle(
      ...
      if (!NoRender.get.isActived() || !NoRender.get.AllParticles.getBool()) {
         this.spawnParticle(
            particleType.getParticleID(), particleType.getShouldIgnoreRange() || ignoreRange, xCoord, yCoord, zCoord, xSpeed, ySpeed, zSpeed, parameters
         );
      }
   }

   private void spawnParticle(
      int particleID, boolean ignoreRange, double xCood, double yCoord, double zCoord, double xSpeed, double ySpeed, double zSpeed, int... parameters
   ) {
      if (!NoRender.get.isActived() || !NoRender.get.AllParticles.getBool()) {
         for (int i = 0; i < this.eventListeners.size(); i++) {
            if (particleID != 7 || Minecraft.player == null || !(Minecraft.player.getDistance(xCood, yCoord + 3.0, zCoord) > 6.0)) {
   ...
   public boolean func_191503_g(Entity p_191503_1_) {
      ...
      return p_191503_1_.posX > d0 && p_191503_1_.posX < d2 && p_191503_1_.posZ > d1 && p_191503_1_.posZ < d3 && !MoveHelper.instance.SkipWorldboder.getBool();
   }
      ...
      while (iterator.hasNext()) {
         ...
         if (!tileentity.isInvalid() && tileentity.hasWorldObj()) {
            ...
            if (this.isBlockLoaded(blockpos) && this.worldBorder.contains(blockpos)) {
               try {
                  this.theProfiler.func_194340_a(() -> String.valueOf(TileEntity.func_190559_a((Class<? extends TileEntity>)tileentity.getClass())));
   ...
   public void updateEntityWithOptionalForce(Entity entityIn, boolean forceUpdate) {
      ...
      if (forceUpdate && entityIn.addedToChunk) {
         ...
         if (entityIn.isRiding()) {
            ...
            entityIn.onUpdate();
            if (!Panic.stop
               && entityIn instanceof EntityLivingBase base
               && (prevPosX != entityIn.posX || prevPosY != entityIn.posY || prevPosZ != entityIn.posZ)) {
               if (JumpCircles.get.actived) {
                  JumpCircles.onEntityMove(base, new Vec3d(prevPosX, prevPosY, prevPosZ));
               }

               DashTrail.dash.onEntityMove(base, new Vec3d(prevPosX, prevPosY, prevPosZ));
            }
         }
      }
   ...
   public boolean handleMaterialAcceleration(AxisAlignedBB bb, Material materialIn, Entity entityIn) {
      ...
      if (!this.isAreaLoaded(j2, l2, j3, k2, i3, k3, true)) {
         ...
         if (vec3d.lengthVector() > 0.0 && entityIn.isPushedByWater()) {
            ...
            double d1 = 0.014;
            entityIn.motionX = entityIn.motionX + vec3d.xCoord * 0.014;
            entityIn.motionY = entityIn.motionY + vec3d.yCoord * 0.014;
            entityIn.motionZ = entityIn.motionZ + vec3d.zCoord * 0.014;
         }
   ...
   public boolean func_190527_a(Block p_190527_1_, BlockPos p_190527_2_, boolean p_190527_3_, EnumFacing p_190527_4_, @Nullable Entity p_190527_5_) {
      ...
      if (axisalignedbb != Block.NULL_AABB && !this.checkNoEntityCollision(axisalignedbb.offset(p_190527_2_), p_190527_5_)) {
         ...
         return iblockstate1.getMaterial() == Material.CIRCUITS && p_190527_1_ == Blocks.ANVIL
            ? true
            : iblockstate1.getMaterial().isReplaceable() && p_190527_1_.canPlaceBlockOnSide(this, p_190527_2_, p_190527_4_);
      }
   }
         ...
         if (j2 >= 15) {
            ...
            if (j2 >= 15) {
               ...
               if (j2 >= 15) {
                  ...
                  return j2 >= 15 ? j2 : Math.max(j2, this.getStrongPower(pos.east(), EnumFacing.EAST));
               }
            }
         }
      }
   ...
   public boolean isBlockPowered(BlockPos pos) {
      if (this.getRedstonePower(pos.down(), EnumFacing.DOWN) > 0) {
         ...
         return this.getRedstonePower(pos.west(), EnumFacing.WEST) > 0 ? true : this.getRedstonePower(pos.east(), EnumFacing.EAST) > 0;
      }
   }
   ...
   public EntityPlayer getNearestAttackablePlayer(BlockPos pos, double maxXZDistance, double maxYDistance) {
      return this.getNearestAttackablePlayer(
         (double)((float)pos.getX() + 0.5F), (double)((float)pos.getY() + 0.5F), (double)((float)pos.getZ() + 0.5F), maxXZDistance, maxYDistance, null, null
      );
   }
   ...
   public boolean isRainingAt(BlockPos strikePosition) {
      if (!this.isRaining()) {
         ...
         return biome.getEnableSnow() ? false : !this.canSnowAt(strikePosition, false) && biome.canRain();
      }
   }
