...
public class BetterSnow {
   ...
   private static boolean hasSnowNeighbours(IBlockAccess p_hasSnowNeighbours_0_, BlockPos p_hasSnowNeighbours_1_) {
      ...
      return p_hasSnowNeighbours_0_.getBlockState(p_hasSnowNeighbours_1_.north()).getBlock() != block
            ...
            && p_hasSnowNeighbours_0_.getBlockState(p_hasSnowNeighbours_1_.east()).getBlock() != block
         ? false
   ...
   private static boolean checkBlock(Block p_checkBlock_0_, IBlockState p_checkBlock_1_) {
      if (p_checkBlock_1_.isFullCube()) {
         ...
         return false;
      } else if (!(p_checkBlock_0_ instanceof BlockBush)
         || !(p_checkBlock_0_ instanceof BlockDoublePlant)
            && !(p_checkBlock_0_ instanceof BlockFlower)
            && !(p_checkBlock_0_ instanceof BlockMushroom)
            && !(p_checkBlock_0_ instanceof BlockSapling)
            && !(p_checkBlock_0_ instanceof BlockTallGrass)) {
         if (p_checkBlock_0_ instanceof BlockFence
            || p_checkBlock_0_ instanceof BlockFenceGate
            || p_checkBlock_0_ instanceof BlockFlowerPot
            || p_checkBlock_0_ instanceof BlockPane
            || p_checkBlock_0_ instanceof BlockReed
            || p_checkBlock_0_ instanceof BlockWall) {
            return true;
         } else if (p_checkBlock_0_ instanceof BlockRedstoneTorch && p_checkBlock_1_.getValue(BlockTorch.FACING) == EnumFacing.UP) {
            return true;
         } else {
