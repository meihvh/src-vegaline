...
public class CustomGuiProperties {
   ...
   public CustomGuiProperties(Properties p_i32_1_, String p_i32_2_) {
      ...
      this.container = (CustomGuiProperties.EnumContainer)connectedparser.parseEnum(
         p_i32_1_.getProperty("container"), CustomGuiProperties.EnumContainer.values(), "container"
      );
      ...
      this.variants = (CustomGuiProperties.EnumVariant[])connectedparser.parseEnums(
         p_i32_1_.getProperty("variants"), acustomguiproperties$enumvariant, "variants", VARIANTS_INVALID
      );
   ...
   private static Map<ResourceLocation, ResourceLocation> parseTextureLocations(
      Properties p_parseTextureLocations_0_,
      String p_parseTextureLocations_1_,
      CustomGuiProperties.EnumContainer p_parseTextureLocations_2_,
      String p_parseTextureLocations_3_,
      String p_parseTextureLocations_4_
   ) {
   ...
   private boolean matchesNameable(BlockPos p_matchesNameable_1_, IBlockAccess p_matchesNameable_2_) {
      if (!(p_matchesNameable_2_.getTileEntity(p_matchesNameable_1_) instanceof IWorldNameable iworldnameable)) {
         return false;
      } else {
   ...
   private boolean matchesBeacon(BlockPos p_matchesBeacon_1_, IBlockAccess p_matchesBeacon_2_) {
      if (!(p_matchesBeacon_2_.getTileEntity(p_matchesBeacon_1_) instanceof TileEntityBeacon tileentitybeacon)) {
         return false;
      } else {
         if (this.levels != null) {
   ...
   private boolean matchesChest(BlockPos p_matchesChest_1_, IBlockAccess p_matchesChest_2_) {
      ...
      if (tileentity instanceof TileEntityChest tileentitychest) {
         ...
         return tileentity instanceof TileEntityEnderChest tileentityenderchest
            ? this.matchesEnderChest(tileentityenderchest, p_matchesChest_1_, p_matchesChest_2_)
            : false;
      }
   }

   private boolean matchesChest(TileEntityChest p_matchesChest_1_, BlockPos p_matchesChest_2_, IBlockAccess p_matchesChest_3_) {
      boolean flag = p_matchesChest_1_.adjacentChestXNeg != null
         || p_matchesChest_1_.adjacentChestXPos != null
         || p_matchesChest_1_.adjacentChestZNeg != null
         || p_matchesChest_1_.adjacentChestZPos != null;
   ...
   private boolean matchesChest(
      boolean p_matchesChest_1_, boolean p_matchesChest_2_, boolean p_matchesChest_3_, boolean p_matchesChest_4_, String p_matchesChest_5_
   ) {
      if (this.large != null && this.large != p_matchesChest_1_) {
         return false;
      } else if (this.trapped != null && this.trapped != p_matchesChest_2_) {
         return false;
      } else if (this.christmas != null && this.christmas != p_matchesChest_3_) {
         return false;
      } else {
         return this.ender != null && this.ender != p_matchesChest_4_ ? false : this.nbtName == null || this.nbtName.matchesValue(p_matchesChest_5_);
      }
   }

   private boolean matchesDispenser(BlockPos p_matchesDispenser_1_, IBlockAccess p_matchesDispenser_2_) {
      if (!(p_matchesDispenser_2_.getTileEntity(p_matchesDispenser_1_) instanceof TileEntityDispenser tileentitydispenser)) {
         return false;
      } else {
         if (this.nbtName != null) {
   ...
   private boolean matchesShulker(BlockPos p_matchesShulker_1_, IBlockAccess p_matchesShulker_2_) {
      if (!(p_matchesShulker_2_.getTileEntity(p_matchesShulker_1_) instanceof TileEntityShulkerBox tileentityshulkerbox)) {
         return false;
      } else {
         if (this.nbtName != null) {
         ...
         if (integer != null && integer1 != null) {
            if (this.professions != null) {
               ...
               for (int i = 0; i < this.professions.length; i++) {
                  ...
                  if (villagerprofession.matches(integer, integer1)) {
                     flag = true;
                     break;
                  }
               }
   ...
   private boolean matchesHorse(Entity p_matchesHorse_1_, IBlockAccess p_matchesHorse_2_) {
      if (!(p_matchesHorse_1_ instanceof AbstractHorse abstracthorse)) {
         return false;
      } else {
         if (this.variants != null) {
         ...
         if (this.colors != null && abstracthorse instanceof EntityLlama entityllama) {
