...
public class Lagometer {
      ...
      if (gameSettings != null && (gameSettings.ofLagometer || gameSettings.showLagometer)) {
         ...
         for (int j = 0; j < timesFrame.length; j++) {
            ...
            if (gcs[j]) {
               ...
               float j1 = f - (float)renderTime(j, timesTick[j], 0, k, k, f, bufferbuilder);
            }
         }
         ...
         mc.fontRendererObj.drawString("30", 2.0F, (double)(k2 + 1), -8947849);
         mc.fontRendererObj.drawString("30", 1.0F, (double)k2, -3881788);
         mc.fontRendererObj.drawString("60", 2.0F, (double)(j2 + 1), -8947849);
         mc.fontRendererObj.drawString("60", 1.0F, (double)j2, -3881788);
         GlStateManager.matrixMode(5889);
         ...
         mc.fontRendererObj.drawString(" " + memMbSec + " MB/s", (float)l1, (double)i2, k1);
   ...
   private static long renderTime(
      int p_renderTime_0_,
      long p_renderTime_1_,
      int p_renderTime_3_,
      int p_renderTime_4_,
      int p_renderTime_5_,
      float p_renderTime_6_,
      BufferBuilder p_renderTime_7_
   ) {
      long i = p_renderTime_1_ / 200000L;
      if (i < 3L) {
         return 0L;
      } else {
         p_renderTime_7_.pos((double)((float)p_renderTime_0_ + 0.5F), (double)(p_renderTime_6_ - (float)i + 0.5F), 0.0)
            .color(p_renderTime_3_, p_renderTime_4_, p_renderTime_5_, 255)
            .endVertex();
         p_renderTime_7_.pos((double)((float)p_renderTime_0_ + 0.5F), (double)(p_renderTime_6_ + 0.5F), 0.0)
            .color(p_renderTime_3_, p_renderTime_4_, p_renderTime_5_, 255)
            .endVertex();
         return i;
      }
   ...
   private static long renderTimeDivider(
      int p_renderTimeDivider_0_,
      int p_renderTimeDivider_1_,
      long p_renderTimeDivider_2_,
      int p_renderTimeDivider_4_,
      int p_renderTimeDivider_5_,
      int p_renderTimeDivider_6_,
      float p_renderTimeDivider_7_,
      BufferBuilder p_renderTimeDivider_8_
   ) {
      long i = p_renderTimeDivider_2_ / 200000L;
      if (i < 3L) {
         return 0L;
      } else {
         p_renderTimeDivider_8_.pos((double)((float)p_renderTimeDivider_0_ + 0.5F), (double)(p_renderTimeDivider_7_ - (float)i + 0.5F), 0.0)
            .color(p_renderTimeDivider_4_, p_renderTimeDivider_5_, p_renderTimeDivider_6_, 255)
            .endVertex();
         p_renderTimeDivider_8_.pos((double)((float)p_renderTimeDivider_1_ + 0.5F), (double)(p_renderTimeDivider_7_ - (float)i + 0.5F), 0.0)
            .color(p_renderTimeDivider_4_, p_renderTimeDivider_5_, p_renderTimeDivider_6_, 255)
            .endVertex();
         return i;
      }
