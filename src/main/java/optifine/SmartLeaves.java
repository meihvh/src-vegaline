...
public class SmartLeaves {
      ...
      if (p_isSameLeaves_0_ == p_isSameLeaves_1_) {
         ...
         if (block != block1) {
            ...
            return block instanceof BlockNewLeaf
               ? p_isSameLeaves_0_.getValue(BlockNewLeaf.VARIANT).equals(p_isSameLeaves_1_.getValue(BlockNewLeaf.VARIANT))
               : false;
         }
      }
   }
   ...
   private static IBakedModel getModelDoubleFace(IBakedModel p_getModelDoubleFace_0_) {
      if (p_getModelDoubleFace_0_ == null) {
         ...
         Config.warn(
            "SmartLeaves: Model is not cube, general quads: "
               + p_getModelDoubleFace_0_.getQuads((IBlockState)null, (EnumFacing)null, 0L).size()
               + ", model: "
