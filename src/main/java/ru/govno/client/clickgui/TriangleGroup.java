...
public class TriangleGroup {
   ...
   private final List<TriangleElement> elements = new ArrayList<>();
   private float x;
   private float y;
   private float w;
   private float h;
   ...
   public static TriangleGroup gen(
      float x, float y, float w, float h, float size, float colorSpeed, long minColorDelay, long maxColorDelay, float chanceToNullColor
   ) {
         ...
         for (int yI = 0; yI < yCount; yI++) {
            ...
            for (int xI = 0; xI < xCount; xI++) {
               ...
               this.elements
                  .add(
                     new TriangleElement(
                        new Vec2f(xOffset + size / 2.0F, yOffset),
                        size,
                        !yReverse,
                        0,
                        0,
   ...
   public void drawAllInZone(float x, float y, float w, float h, float alphaPC, boolean bloom) {
      List<TriangleElement> filteredElements = this.elements.stream().filter(element -> {
         ...
         float xPos = (isReversed ? element.getOffsetVectors().get(0).x : element.getOffsetVectors().get(2).x) - size;
         float yPos = (isReversed ? element.getOffsetVectors().get(2).y : element.getOffsetVectors().get(1).y) - size;
      ...
      if (!filteredElements.isEmpty()) {
         ...
         filteredElements.forEach(
            element -> element.getOffsetVectors()
                  .forEach(vector -> this.bufferBuilder.pos((double)vector.x, (double)vector.y).color(element.getColor(alphaPC)).endVertex())
         );
         this.tessellator.draw();
         GL11.glLineWidth(0.825F * lpSCFactor);
         ...
         this.bufferBuilder.begin(0, DefaultVertexFormats.POSITION_COLOR);
         filteredElements.forEach(
            element -> element.getOffsetVectors()
                  .forEach(vector -> this.bufferBuilder.pos((double)vector.x, (double)vector.y).color(element.getColor(alphaPC)).endVertex())
         );
