...
public class InventoryUtil {
   ...
   public static boolean isIntercepted(BlockPos pos) {
      for (Entity entity : Minecraft.getMinecraft().world.loadedEntityList) {
         if (new AxisAlignedBB(pos).intersectsWith(entity.getEntityBoundingBoxCLCompact())) {
            return true;
         }
      }
   ...
   public static boolean isPotion(ItemStack stack, Potion potion, boolean splash) {
      if (stack == null) {
         return false;
      } else if (stack.getItem() instanceof ItemPotion potionItem) {
   ...
   public static int getSwordSlot() {
      ...
      for (int i = 9; i < 45; i++) {
         if (Minecraft.player.inventoryContainer.getSlot(i).getHasStack()) {
            ...
            if (item != null) {
               Item damage = item.getItem();
               if (damage instanceof ItemSword) {
                  ItemSword is = (ItemSword)damage;
                  float damagex = is.getDamageVsEntity();
                  float var7;
                  if ((
                        var7 = damagex
   ...
   public static int getBestSword() {
      ...
      for (int k = 0; k < Minecraft.player.inventory.mainInventory.size(); k++) {
         ...
         if (is != null) {
            Item damage = is.getItem();
            if (damage instanceof ItemSword) {
               ItemSword itemSword = (ItemSword)damage;
               float damagex = (float)itemSword.getMaxDamage();
               float var7;
               if ((var7 = damagex + (float)EnchantmentHelper.getEnchantmentLevel(Enchantment.getEnchantmentByID(20), is)) > bestDamage) {
                  bestDamage = var7;
                  bestSword = k;
               }
            }
