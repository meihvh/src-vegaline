...
public class ShadowUtils {
   public static Iterator<RenderChunk> makeShadowChunkIterator(
      ...
      if (f > 0.0F && f < (float)((renderDistanceChunks - 1) * 16)) {
         ...
         float f1 = Shaders.sunPathRotation * (float) (Math.PI / 180.0);
         float f2 = f6 > (float) (Math.PI / 2) && f6 < (float) (Math.PI * 3.0 / 2.0) ? f6 + (float) Math.PI : f6;
         float f3 = -MathHelper.sin(f2);
         float f4 = MathHelper.cos(f2) * MathHelper.cos(f1);
         float f5 = -MathHelper.cos(f2) * MathHelper.sin(f1);
         BlockPos blockpos = new BlockPos(
            MathHelper.floor(viewEntity.posX) >> 4, MathHelper.floor(viewEntity.posY) >> 4, MathHelper.floor(viewEntity.posZ) >> 4
         );
